...
_This third article on [Q][1] is a little parenthesis to the Qep articles series,
featuring the `requestAnimationFrame` Javascript function and its general usage,
and [QanimationFrame][1], its Promisified version used as a "wait for DOM to be ready" API._
...
_(N.B. there is not anymore a second DOM parameter like a few months ago, see the [spec][2])_
...
given in second parameters _(sometimes 30, sometimes 0 !?)_.
...
**N.B.:** Even if `requestAnimationFrame` doesn't have anymore a second _DOM element_ parameter,
...
var height = Q.fcall(createDivInBody, "Hello world!<br/>How are you today?")
