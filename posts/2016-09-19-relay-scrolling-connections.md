...
[Relay][relay] doesn't solve for you how you should render your components. Relay is "universal" and doesn't even assume it will be running in a browser context. It focuses only on providing an abstraction to work with GraphQL – the same way React focuses only on rendering. Each library solves one single problem at a time _(and hell, both are complex enough problem to solve already)_.
...
```js
<InfiniteScrollable relay={relay}>...</InfiniteScrollable>
```
