...
You have to make a very hard choice: **_Which feature to remove?_**
...
**_JSCrush_** is a crazy tool you may want to use to go deeper in the bytes reducing.
...
this decompressor might be an overhead _(~ +60 bytes with small code)_.

If you are using _JSCrush_ which I recommend for saving extra bytes,
...
But if you optimize your code **for** _JSCrush_, you can save much more!
...
```javascript
size = a + b + 10;
fillRect(x - size, y - size, 2 * size, 2 * size); // no please don't!
fillRect(
  x - (a + b + 10),
  y - (a + b + 10),
  2 * (a + b + 10),
...
_Special thanks to [@mrspeaker](http://twitter.com/mrspeaker) for fixing my English_.
