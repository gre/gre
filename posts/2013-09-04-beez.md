...
_Bonus for the one who recognizes the melody :-)_
...
The **Hive** is a web page where the sound is generated and visualized _(Web Audio API)_.
...
It only works on Android Chrome now _(WebRTC required)_.
...
We have a note sequencer which plays a **_famous melody_** through a [Frequency Modulator][fm_article] and different other effects.
...
_As you can see, we used Backbone.js models for events._
...
- Not always easy to traverse **proxies**. _(e.g. through an nginx front server)_
- Only text communication.
- **bandwith** intensive. _(all the bandwidth goes back and forth with your server)_
- **CPU** intensive. _(e.g. receiving 20 messages per second from 10 clients can be a lot for a small server, especially if you are doing some message processing)_
...
[WebRTC][webrtc] (_Web Real Time Communication_),
...
That resolving phase requires a communication between the clients, and for that we can use WebSockets as a _"Control Channel"_.
