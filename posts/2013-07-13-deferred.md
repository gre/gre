...
_This second article on [Q][1] will introduce you how to easily
...
with the implementation of [Qimage][2], a simple Image Promise wrapper._
...
- `.resolve(value)`: moving from _pending_ to **_fulfilled_ with a value**.
- `.reject(error)`: moving from _pending_ to **_rejected_ with an error**.
...
**_N.B._**: _this separation also exists in other languages but with different names (for instance in Scala: Promise / Future)._

### `Q.defer()`

The method Q.defer() will return a new **Deferred object** initialized in a _pending_ state.
...
```javascript
Q.all([Qimage("res1.png"), Qimage("res2.png"), Qimage("res3.png")]).spread(
...
_a fundamental function generally used for performing efficient Javascript animations_.
