...
- (ex block 12978465) txCountLightFactor: the number of value transferred is expectionally low in the block. => there will not be a lot of noise.
...
if (allNonZeroValues.length > 50) {
    ...
    const amps = [
      ...
      1 - txCountLightFactor,
      1 + (5 * rng.random() - 0.5) * (1 + 2 * rng.random()) +
      30 * expectionalTxAmountFactor
    ]
    const multicolor = rng.random() * (
    ...
    const lowcutoff = rng.random() * rng.random();
    return {
      amps,
      shapearg,
      fbmoffset,
...
const shaders = Shaders.create({
  main: {
    ...
    float scene(in vec2 p, float t) {
      vec2 q = vec2( fbm( amps[0] * p ), fbm( amps[0] * p + vec2(3.1,3.32) ) );
      vec2 r = vec2( fbm( 0.2 * amps[1] * q ),
                    fbm( amps[1] * q) );
