...
fn art(opts: &Opts) -> Vec<Group> {
  ...
  for i in 0..count {
    ...
    let density = rng.gen_range(1.5, 2.5);
    let iterations = rng.gen_range(800, 1000);
    let clr = 0;
  ...
  let yincrlayer = rng.gen_range(10.0, 14.0);
  let mut y = ystart;
  let mut w = w;
  ...
  let ptowers = rng.gen_range(0.0, 1.0) * rng.gen_range(0.0, 1.0);
  let pchapel = rng.gen_range(0.0, 0.2) * rng.gen_range(0.0, 1.0);
  let pdarkchapel = rng.gen_range(0f64, 1.0).powf(2.0);
  let pwall = rng.gen_range(0.8, 1.0);

  let count = rng.gen_range(15, 20);
  for yi in 0..count {
    let scale = 1.0 + scalef / (yi as f64 + 1.0);
    let inter = interbase + interincr * yi as f64;
    let splits = if yi == 0 { 1 } else { rng.gen_range(1, 6) };
  ...
  routes.extend(sun(&mut paint, c, height.max(width) * 0.7, 2.0, 2));
...
pub fn army_fighter<R: Rng>(
  ...
  let joints = HumanJointAngles {
    body_angle: -PI / 2.0 + rng.gen_range(-0.5, 0.5) * rng.gen_range(0.0, 1.0),
    head_angle: -PI / 2.0 + rng.gen_range(-0.5, 0.5) * rng.gen_range(0.0, 1.0),
...
fn head(
  ...
  // translate and rotate routes
  regular_clip_polys(&vec![(clr, route)], paint, &polys)
}
