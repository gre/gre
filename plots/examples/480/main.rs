...
pub struct Opts {
  ...
  pub height: f64,
  #[clap(short, long, default_value = "73.0")]
  pub seed: f64,
...
fn art(opts: &Opts) -> Vec<Group> {
  ...
  let track_count = 28;
  let cordon_w = 30.0;
  let noise_freq = rng.gen_range(0.06, 0.16);
  let noiseamp = rng.gen_range(6.0, 12.0);
  let round = 4.0;
  ...
  colors
    ...
    .map(|(_i, color)| {
      ...
      // TODO we can iterate multiple time to push points when angle is too extreme (ping pong)
...
fn cordon(
  ...
  for &next in path.iter().skip(1) {
    // if we rotate > 90°, reverse tracks, to bounce
    let dx = next.0 - p.0;
    let dy = next.1 - p.1;
    let a = dy.atan2(dx);
    let mut i = 0.0;
