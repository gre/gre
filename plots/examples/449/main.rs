...
fn art(opts: &Opts) -> Vec<Group> {
    ...
    let max_passage = 8;
    let min_c = 1.0 + rng.gen_range(0.0, 20.0) * rng.gen_range(0.0, 1.0);
    let max_c = min_c + rng.gen_range(1.0, 100.0) * rng.gen_range(0.0, 1.0);
    let secondary_threshold = rng.gen_range(20.0, 100.0);
    let packing_padding = 2.0 + rng.gen_range(0.0, 30.0) * rng.gen_range(0.0, 1.0);
    let circle_bounds = (-0.4 * width, -0.4 * height, 1.4 * width, 1.4 * height);
