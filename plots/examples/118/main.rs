...
fn art(opts: Opts) -> Vec<Group> {
  ...
  for pass in 0..count {
    ...
    for i in 0..(samples + 1) {
      ...
      let mut p = (width * 0.5 + size * o.0, height * 0.5 + size * o.1);
      let noise_angle = perlin.get([0.05 * p.0, 0.05 * p.1, 100.0 + opts.seed]);
      let noise_amp = 2. * perlin.get([0.01 * p.0, 0.01 * p.1, opts.seed]);
