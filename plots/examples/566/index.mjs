...
let MAX_MOUNTAIN_LAYERS = 5;
let MOUNTAIN_DENSITY = 3;
let MAX_SUN_RADIUS = 40;
let SUN_DENSITY = 10;
let MAX_GROUP_OF_BIRDS = 2;
let MAX_BOATS = 6;
let REFLECTION_PROBABILITY = 0.08;
...
function art(S) {
  ...
  let mountains_delta = Math.min(40, (30 + rand(50)) / mountains); // defines the "stops" of each mountains layer
  for (let i = 0; i < mountains; i++) {
    ...
    let amp1 = 0.5 / f1;
    let amp2 = 0.3 + rand(0.2);
    let amp3 = 0.6 + rand(0.5);
    // "s" is for the second perlin noise added to the first
    let sf1 = 0.005 + rand(0.005);
    let sf2 = 0.005 + rand(0.01);
    let samp2 = 3 + rand(5);
    let ystop = 0.5 * HEIGHT - (i + 1) * mountains_delta;
