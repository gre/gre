...
fn art(opts: &Opts) -> Vec<Group> {
  ...
  let freq2 = rng.gen_range(2.0, 4.0)
    * mix(freq1, rng.gen_range(0.01, 0.08), rng.gen_range(0.0, 1.0));
  let amp2 = rng.gen_range(0.0, 4.0);
  let freq3 = rng.gen_range(4.0, 16.0)
    * mix(freq1, rng.gen_range(0.01, 0.08), rng.gen_range(0.0, 1.0));
  ...
  let angle_mul =
    (rng.gen_range(-10f64, 60.0) * rng.gen_range(0.0, 1.0)).max(0f64);
  ...
  loop {
    ...
    loop {
      ...
      let hba_index = (highest_by_angle.len() as f64 * (a / 2. * PI)).round()
        as usize
      ...
      r += amp1
        * base_r
        * (amp_r_const + base_r / max_r).powf(amp1pow)
        * perlin.get([
          -opts.seed
            + amp2
              * perlin.get([
