...
fn art(opts: &Opts) -> Vec<Group> {
  ...
  for i in 0..count {
    let xincr = 1.0;
    let y = height - pad;
    let yamp = height * rng.gen_range(0.4, 0.7);
    let perlin = Perlin::new();
    let density = rng.gen_range(1.8, 5.4);
    let iterations = (rng.gen_range(0.2, 1.0) * width * height / 30.0) as usize;
    let clr = 0;

    let f1 = rng.gen_range(0.01, 0.05) * rng.gen_range(0.2, 1.0);
    let amp2 = rng.gen_range(0.2, 2.0) * rng.gen_range(0.1, 1.0);
    let f2 = rng.gen_range(0.02, 0.1) * rng.gen_range(0.1, 1.0);
    let amp3 = rng.gen_range(0.1, 1.0) * rng.gen_range(0.1, 1.0);
    let f3 = rng.gen_range(0.1, 0.2) * rng.gen_range(0.1, 1.0);
  ...
  let r = rng.gen_range(0.3, 0.4) * width.min(height);
  routes.extend(sun(&mut paint, c, r, 0.6, 2));
  routes.extend(sun(&mut paint, c, width.max(height), 2.0, 2));
