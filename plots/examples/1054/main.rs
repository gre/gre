...
fn art(opts: &Opts) -> svg::Document {
  ...
  let inks = vec![pink, amber];

  let perlin = Perlin::new();

  let mut routes = Vec::new();

  let f1 = rng.gen_range(1.0, 10.0);
  let amp1 = 1.0;
  let f2 = f1 * rng.gen_range(1.0, 3.0);
  let amp2 = rng.gen_range(0.0, 6.0);
  ...
  for (ink_i, _ink) in inks.iter().enumerate() {
    ...
    map.fill_fn(&mut |(x, y)| {
      let x = (x - width / 2.0).abs() + width / 2.0;
      ...
      let r = 16.0;
      let valuemod = ((dx + dy) / r) % 2.0;
      let colorf = ink_i as f64;
      if !(colorf < valuemod && valuemod < colorf + 1.0) {
        return 0.0;
      }
      let value2 = valuemod % 1.0;
