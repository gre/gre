...
fn art(opts: &Opts) -> svg::Document {
  ...
  let inks = vec![(fire_and_ice, 6.0, 0.45)];
  let get_color = image_get_color("images/soulvertex15.jpg").unwrap();
  ...
  for (ink_i, (_ink, density, weight_map_prec)) in inks.iter().enumerate() {
    ...
    map.fill_fn(&mut rng, &mut |(x, y): (f64, f64), _rng| {
      if x < pad || x > width - pad || y < pad || y > height - pad {
        return 0.0;
      }

      let clr = get_color((x / width, y / height));
      density * (1.0 - 0.9 * grayscale(clr).powf(2.0))
    });
