...
fn art(opts: &Opts) -> svg::Document {
  ...
  */
  ...
  let inks = vec![black, black, black];
  ...
  let count_imgs =
    (1.0 + rng.gen_range(0.0, 5.0) * rng.gen_range(0.0, 1.0)) as usize;
  ...
  let collages = (0..(count_imgs
    + (rng.gen_range(0.0, 10.0) * rng.gen_range(0.0, 1.0)) as usize))
    .map(|i| {
      (
        i % images.len(),
  ...
  map.fill_fn(|(x, y)| {
    ...
    for (index, v, s, r, repeat, multiplier) in collages.clone() {
      ...
      let value = 1.0 - grayscale(c);
      let value = value * 1.1 - 0.2;
