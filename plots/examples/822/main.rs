...
fn art(opts: &Opts) -> Vec<Group> {
  ...
  let incr = rng.gen_range(0.5, 0.8);
  let mut p = xpad;
  let mut i = 0;
  let rep = rng.gen_range(1, 24);
  loop {
    if p > width - xpad {
      break;
    }
    let clr = (i / rep) % 2;
    routes.push((clr, vec![(p, pad), (p, height - pad)]));
    p += incr;
    i += 1;
  }

  let count =
    1 + (rng.gen_range(20.0, 100.0) * rng.gen_range(0.0, 1.0)) as usize;
  let split = rng.gen_range(0.0, 0.5);
  for _i in 0..count {
    let x = width * rng.gen_range(0.4, 0.6);
    let y = height * rng.gen_range(0.1, 0.9);
    let a = 0.2
      * rng.gen_range(-PI, PI)
    ...
    let slide =
      0.03 * width * rng.gen_range(-1.0, 1.0) * rng.gen_range(0.0, 1.0);
