...
fn art(opts: &Opts) -> Vec<Group> {
  ...
  let passes = rng.gen_range(4, 8);
  let min_scale = rng.gen_range(0.0, 0.2) * rng.gen_range(0.0, 1.0) * height;
  let max_scale =
    min_scale + rng.gen_range(0.0, 0.5) * rng.gen_range(0.0, 1.0) * height;
  ...
  let freq = rng.gen_range(0.0, 0.08) * rng.gen_range(0.0, 1.0);
  let disp = rng.gen_range(0.0, 0.5);
  let dispersion = 0.3;
  for (i, &c) in circles.iter().enumerate() {
    ...
    route = route
      ...
      .collect();
    let modlength = rng.gen_range(10, 100);
    let modl =
      (modlength as f64 * rng.gen_range(0.2f64, 1.5).min(1.0)) as usize;
    if modl == modlength {
      routes.push((clr, route));
    } else {
      for route in route
        .chunks(modlength)
        .map(|r| r.iter().take(modl).map(|&p| p).collect())
      {
        routes.push((clr, route));
      }
    }
