...
fn art(opts: &Opts) -> Vec<Group> {
  let colors = vec!["darkgrey", "darkcyan"];
  let seed = opts.seed;
  ...
  let target_size = rng.gen_range(20, 40);
  let particles = 500000 / target_size;
  let mut layers = Vec::new();
  let mut passage = Passage2DCounter::new(0.5, width, height);
  let max_passage = 6;
  let gridw = (1. + rng.gen_range(0., 100.) * rng.gen_range(0.0, 1.0)) as i32;
  let gridh = (1. + rng.gen_range(0., 100.) * rng.gen_range(0.0, 1.0)) as i32;
  let min_threshold = rng.gen_range(4.0, 16.0);
  ...
  let mut circles = packing(
    ...
    min_threshold,
    rng.gen_range(2.0 * min_threshold, 3. * min_threshold),
  );

  rng.shuffle(&mut circles);
  let ampang = rng.gen_range(0.0, 3.0);
  let f1 = rng.gen_range(1.0, 3.0);
  let f2 = rng.gen_range(1.0, 3.0);
    ...
    let routes: Vec<Vec<(f64, f64)>> = samples
      ...
      .map(|(si, &sample)| {
        ...
        loop {
          ...
          let dycenter = 2.0 * (g.1 - height / 2.0).abs() / height;
          ang += dycenter
            + 1.4 * perlin.get([xi as f64 * 0.3, yi as f64 * 0.3, seed]).abs();
