...
fn art(opts: &Opts) -> Vec<Group> {
    ...
    let f = |(x, y): (f64, f64)| {
        ...
        let amp2 = opts.amp2
            * (0.5
                + 0.5 * ((progress + x) * 2. * PI).cos());
        let freq2 = opts.freq2;
        let amp3 = opts.amp3
            * (0.5
                + 0.5 * ((progress + x) * 2. * PI).sin());
        let freq3 = opts.freq3;
        let amp4 = opts.amp4;
        let freq4 = opts.freq4;

        -0.6 + 5.5 * euclidian_dist((x, y), (0.5, 0.5))
            + perlin.get([
                ratio * freq1 * x
                    + amp2
                        * perlin.get([
    ...
    let mut thresholds: Vec<f64> = (0..opts.layers)
        .map(|i| i as f64 * opts.mult)
        .collect();
    thresholds
        .push(thresholds[thresholds.len() - 1] + 0.001);
