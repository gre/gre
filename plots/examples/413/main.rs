...
fn rec_packing(
    ...
    let mut rng = rng_from_seed(seed);
    let newci = if rng.gen_bool(
        (main_probability - i as f64 / 8.).max(0.)
    ) {
        (ci + 1) % 2
    } else {
        ci
    };
    let mut pad = 0.1 + 0.3 * ((i % 4) as f64);
    let min = 0.2 + 0.5 * pad;
    if i == 0 {
        pad += 1.0;
    }
    ...
    let circles: Vec<(usize, VCircle)> =
      primaries.iter().map(|&c| (newci, c)).collect();
...
fn art(opts: Opts) -> Vec<Group> {
    ...
    let main_probability = rng.gen_range(0.2, 0.8);
    let retries = rng.gen_range(1, 60);
    let container_boundaries = (pad, pad, width - pad, height - pad);
    let bounds_container = VCircle::new(width/2.0, height/2.0, height + width);
    let circles = rec_packing(0, 1, opts.seed, container_boundaries, &bounds_container, main_probability, retries);

    println!("{} circles", circles.len());
    println!("{} retries", retries);
    println!("{} main_probability", main_probability);
