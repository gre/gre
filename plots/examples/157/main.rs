...
fn art(opts: Opts) -> Vec<Group> {
  ...
  let get_angle =
    |initial_angle, length, j| initial_angle + 0.3 - length as f64 * 0.005;

  let perlin = Perlin::new();

  let parametric = |t: f64| {
    (
      1.5 * (2. * PI * t).cos() + 0.3 * (6. * PI * t).cos(),
      1.1 * (2. * PI * t).sin() + 0.3 * (6. * PI * t).sin(),
    )
  };
  ...
  let parametric2 = |t| {
    ...
    let n = perlin.get([
      opts.freq * x,
      opts.seed + 0.1 * perlin.get([opts.seed, 2. * opts.freq * x]),
    ]);
    return (
      0.7 * p.0 - 0.3 * (504. * PI * t).cos(),
      0.5 * p.1
        + 0.2 * (6. * PI * t).sin()
        + 0.02 * (1000. * PI * t).sin()
        + opts.amp * n,
  ...
  let mut build_route = |p, i, j| {
    ...
    let rep = 40;
    if j % 2 == 0
      && (i + 8 * j + (rep as f64 * (1. - opts.offset)) as usize) % rep == 0
    {
  ...
  let mut second: Vec<(f64, f64)> = (0..opts.samples2)
    .map(|i| {
  ...
  let all = vec![routes, vec![second]];

  let mut passage = Passage2DCounter::new(0.5, width, height);
  let max_passage = 3;
  let mut should_draw_line = |a, _b| passage.count(a) < max_passage;
  ...
  colors
    ...
    .map(|(i, &color)| {
      ...
      if i == 0 {
        ...
        g = g.add(signature(1.0, (252.0, 190.0), color))
      }
      return g;
    })
    .collect()
