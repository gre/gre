...
pub struct Opts {
  ...
  pub margout: f64,
  #[clap(short, long, default_value = "13370.0")]
  pub seed: f64,
...
fn art(opts: &Opts) -> Vec<Group> {
  ...
  let white: Ink = Ink("White", "#fff", "#eee", 0.35);
  ...
  let noisefactor = rng.gen_range(0.0, 0.5) * rng.gen_range(0.0, 1.0);
  let noisebalance = rng.gen_range(0.0, 0.5);
  let ang1 = if rng.gen_bool(0.3) {
    rng.gen_range(-PI, PI)
  } else {
    0.0
  };
  let ang2 = if rng.gen_bool(0.3) {
    rng.gen_range(-PI, PI)
  } else {
    ang1
  };
  let ang3 = if rng.gen_bool(0.3) {
    rng.gen_range(-PI, PI)
  } else {
    ang2
  };
  ...
  let coloring = |_route: &Vec<(f64, f64)>| 0;
