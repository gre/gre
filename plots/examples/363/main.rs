...
fn art(opts: Opts) -> Vec<Group> {
  ...
  let routes = circles
    ...
    .flat_map(|circle| {
      ...
      let choice = (rng.gen_range(0.0, 1.0) * rng.gen_range(0.0, 1.0) * 7f32)
        .floor() as u8;
      match choice {
...
fn shape_cross_hatch(
  seed: f64,
  _opts: &Opts,
  c: &VCircle,
) -> Vec<Vec<(f64, f64)>> {
  let mut rng = rng_from_seed(seed);
  let mut points: Vec<(f64, f64)> = (0..((20. + 1.2 * c.r * c.r) as usize))
    .map(|_i| {
...
fn shape_cross_x(seed: f64, _opts: &Opts, c: &VCircle) -> Vec<Vec<(f64, f64)>> {
  let mut rng = rng_from_seed(seed);
  let mut points: Vec<(f64, f64)> = (0..((20. + 2. * c.r * c.r) as usize))
    .map(|_i| {
      let a = rng.gen_range(0., 2. * PI);
      (c.x + c.r * a.cos(), c.y + c.r * a.sin())
    })
...
fn shape_strokes_spiral(
  ...
  result.push(p);
  loop {
    samples = samples.into_iter().filter(|&x| x != p).collect();
