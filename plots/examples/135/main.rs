...
fn art(opts: &Opts) -> Vec<Group> {
  ...
  let f = |(x, y)| {
    offset
      ...
      + perlin.get([
        ratio * freq1 * x
          + amp2 * perlin.get([ratio * freq2 * x, freq2 * y, 50. + seed]),
        freq1 * y
          + amp2 * perlin.get([ratio * freq2 * x, freq2 * y, 100. + seed]),
        seed + amp3 * perlin.get([ratio * freq3 * x, freq3 * y, 10. + seed]),
      ])
  };
  let thresholds = (0..opts.count).map(|i| i as f64 * opts.mult).collect();
