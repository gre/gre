...
fn art(opts: &Opts) -> Vec<Group> {
  ...
  let scale = 1.0;
  let xdisp = rng.gen_range(6.0, 15.0);
  let ydisp = xdisp * rng.gen_range(0.0, 0.7);
  let disp = rng.gen_range(0.0, 0.1) * ydisp * rng.gen_range(0.0, 1.0);
  let w = 50.0;
  let h = 20.0;
  let dx = rng.gen_range(0.0, 0.3) * w;
  let divs = rng.gen_range(6, 16);
  let lcount = rng.gen_range(20, 30);

  let project_polyline = |l: &Vec<(f64, f64)>| -> Vec<(f64, f64)> {
    l.iter()
      .map(|(x, y)| (scale * x + width / 2.0, scale * y + height / 2.))
      .collect()
  };

  let mut lines = vec![];

  let xbase = xdisp * (0.4 + rng.gen_range(0.0, 2.0) * rng.gen_range(0.0, 1.0));
  let ybase = (height / 2.0 - pad) * rng.gen_range(0.5, 0.8);
  for i in 0..lcount {
    let angle = (i as f64 / (lcount as f64)) * 2. * PI;
    let acos = angle.cos();
    let asin = angle.sin();
    let x = xbase + xdisp * acos + rng.gen_range(-disp, disp);
    let y = ybase + ydisp * asin + rng.gen_range(-disp, disp);
    let line = vec![(x, y), (-x, -y)];
    lines.push((1, line));
  }
  ...
  let lines = clip_routes_with_colors(
    &lines,
    &|p| 0.0 < p.1 && p.1 < h / 2.0 + 0.5,
    1.0,
    4,
  );
  let lines: Vec<(usize, Vec<(f64, f64)>)> = lines
    .iter()
    .map(|(c, l)| (*c, project_polyline(l)))
    .collect();

  for (_, l) in &lines {
    paint.paint_polyline(l, 0.5);
  }
  ...
  for xi in 0..divs + 1 {
    let x = (xi as f64 / divs as f64 - 0.5) * w;
    let x1 = x - dx;
    let x2 = x + dx;
    let y1 = -h / 2.0;
    let y2 = h / 2.0;
    let line = vec![(x1, y1), (x2, y2)];
    grids.push((0, line));
  }
  for yi in 0..divs + 1 {
    let p = yi as f64 / divs as f64 - 0.5;
    let y = p * h;
    let y1 = y;
    let y2 = y;
    let offx = 2.0 * p * dx;
  ...
  let grids = grids
    .iter()
    .map(|(c, l)| (*c, project_polyline(l)))
    .collect::<Vec<(usize, Vec<(f64, f64)>)>>();

  let grids = clip_routes_with_colors(&grids, &|p| paint.is_painted(p), 1.0, 4);

  routes.extend(grids);

  let p = 0.5;
  paint.paint_polygon(&project_polyline(&vec![
    (-w / 2.0 - dx - 2. * p, -h / 2.0 - 2. * p),
    (w / 2.0 - dx + 2. * p, -h / 2.0 - 2. * p),
    (w / 2.0 + dx + 2. * p, h / 2.0 + 2. * p),
    (-w / 2.0 + dx - 2. * p, h / 2.0 + 2. * p),
  ]));
  ...
  let mut y = pad;
  while y < height - pad {
    rts.push((3, vec![(pad, y), (width - pad, y)]));
    y += 4.0;
  }
  let rts = clip_routes_with_colors(&rts, &|p| paint.is_painted(p), 1.0, 4);
  routes.extend(rts);

  let mut clrs = vec!["#000", "#f33", "#09F", "#F90"];
  if rng.gen_bool(0.2) {
