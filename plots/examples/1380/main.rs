...
fn art(opts: &Opts) -> Vec<Group> {
  ...
  let params = vec![
    (rng.gen_range(2, 7), rng.gen_range(0.4, 0.6)),
    (rng.gen_range(30, 50), rng.gen_range(0.0, 0.3)),
    (rng.gen_range(200, 400), rng.gen_range(0.0, 0.15)),
  ];

  polygons.push(vec![(0., 0.), (width, 0.0), (width, height), (0., height)]);

  for (count, proba) in params {
    let mut points = vec![];
    for _i in 0..count {
    ...
    let dcel = voronoi::voronoi(points, width);
    for poly in voronoi::make_polygons(&dcel) {
  ...
  routes.extend(filling.fill(
    ...
    &|x, y| {
      if polygons
        .iter()
        .filter(|poly| polygon_includes_point(poly, (x, y)))
        .count()
        % 2
        == 1
      {
