  ...
  colors
    ...
    .map(|(ci, &color)| {
      ...
      let f = |p: (f64, f64)| {
        ...
        2.0
          * noise.get([
            x + rng.gen_range(0.0, 8.0)
              * noise.get([
                rng.gen_range(0.0, 20.0) * x,
                rng.gen_range(0.0, 10.0) * y,
                66. + 7.4 * seed,
              ]),
            y + rng.gen_range(0.0, 8.0)
              * noise.get([
                rng.gen_range(0.0, 20.0) * x,
                rng.gen_range(0.0, 10.0) * y,
                6. + 9.4 * seed
                  + 2.0
                    * rng.gen_range(0.0, 1f64).powf(2f64)
                    * noise.get([
      ...
      for i in 0..count {
        ...
        for c in 0..circles {
          ...
          for s in 0..splits {
            ...
            let p = (width / 2.0 + a.cos() * r, height / 2.0 + a.sin() * r);
            let v = f(p) * (x - 0.5);
            let rx = r + size * v;
            let p = (width / 2.0 + a.cos() * rx, height / 2.0 + a.sin() * rx);
            route.push(p);
          }
          routes.push(route);
        }
