...
fn bg_shape<R: Rng>(
  ...
  for i in 0..count {
    let pr = i as f64 / (count as f64 - 1.);
    main.push((
      rng.gen_range(-scale, scale) * 0.5 * rng.gen_range(0.0, 1.0),
      mix(-scale, scale, pr),
    ));
  }
  main = path_subdivide_to_curve(main, 2, 0.7);
  for _i in 0..3 {
    let mut l = main.clone();
    l = shake(l, 0.06 * scale, rng);
    routes.push(l);
  }

  let perlin = Perlin::new();
  let angmul = rng.gen_range(0.4, 1.3);

  let count = (scale * 8.0) as usize;
  let vx = rng.gen_range(0.0, 0.2);
  for i in 0..count {
    let pr = i as f64 / (count as f64 - 1.);
    let mut v = pr * ((main.len() - 1) as f64);
    v = mix(v, 0.0, vx);
    let index = v.floor() as usize;
    let pl = v - v.floor();
    let p = lerp_point(main[index], main[(index + 1).min(main.len() - 1)], pl);
    let mut line = Vec::new();
    for (a, am) in vec![(-1.0, 1.0), (0.0, 0.0), (1.0, 1.0)] {
      let ang = -PI / 2.0
        + a * angmul
      ...
      let amp = am
        * scale
        * mix(0.1, 0.5, pr.powf(0.5))
        * (1.0
          + 0.2
            * perlin.get([p.0 * 0.67, p.1 * 0.71, origin.0 + 77. * origin.1]));
      line.push((p.0 + amp * ang.cos(), p.1 + amp * ang.sin()));
    }
    routes.push(path_subdivide_to_curve(line, 1, 0.7));
  }
...
fn fowl<R: Rng>(
  ...
  let count = (scale * rng.gen_range(1.8, 2.2)) as usize;
  let mut path = Vec::new();
  for (i, l) in lead.iter().enumerate() {
    let percent = i as f64 / ((lead.len() - 1) as f64);
    for _i in 0..count {
  ...
  let jumpstop =
    (1.0 - 0.3 * rng.gen_range(0.0, 1.0) * rng.gen_range(0.0, 1.0)) * jump;
  for _i in 0..3 {
    let mut path = Vec::new();
    let mut y = 0.0;
    let f = rng.gen_range(0.0, 30.0) * rng.gen_range(0.0, 1.0);
    let amp1min = rng.gen_range(0.0, 1.0);
    let amp2max = rng.gen_range(0.0, 0.1);
    let base = rng.gen_range(0.4, 0.6);
    let min = 6;
    let proba = 0.25;
    let incr = 0.05;
    loop {
      ...
      let amp = 0.3 * mix(1.0, amp1min, y / jump).powf(2.0);
      let amp2 = mix(amp2max, 0.0, y / jump);
      let realy = base + 1.3 * y / scale;
      let p = (
        amp * (realy * f).cos(),
        realy + amp2 * (realy * f).sin().abs(),
      );
      path.push(p);
      ...
      (
        ...
        route
          ...
          .map(|&p| {
            let mut p = p_r(p, rot);
            p = round_point(
              (scale * p.0 + origin.0, scale * (p.1 - dy) + origin.1),
              0.01,
            );
            passage.count(p);
            p
...
fn bad_dog<R: Rng>(
  ...
  let count = (rng.gen_range(0.5, 5.0) * scale) as usize;
  let mut candidates: Vec<(f64, f64)> = (0..count)
    .map(|_i| {
      (
  ...
  let mut route = path_subdivide_to_curve(
    tsp(candidates, time::Duration::seconds(1)),
    1,
    0.8,
  );
  route.push(route[0]);
    ...
    .map(|route| {
      (
        ...
        route
          ...
          .map(|&p| {
            let p = round_point(
              (scale * p.0 * xmul + origin.0, scale * p.1 + origin.1),
              0.01,
            );
            p
          })
...
fn art(opts: &Opts) -> Vec<Group> {
  ...
  let precision = 0.2;
  let count = (2. + rng.gen_range(0., 20.) * rng.gen_range(0.2, 1.0)) as usize;
  for j in 0..count {
    let stopy = rng.gen_range(0.4, 0.7) * height;
    let peakfactor = rng.gen_range(-0.0008, 0.002)
      * rng.gen_range(-0.3f64, 1.0).max(0.0)
      * rng.gen_range(j as f64 / (count as f64), 1.0);
    let ampfactor = rng.gen_range(-0.01f64, 0.1).max(0.0);
    let ynoisefactor = rng.gen_range(0.02, 0.2);
    let yincr = rng.gen_range(0.5, 1.0);
    let amp2 = rng.gen_range(1.0, 12.0);
    let offsetstrategy = rng.gen_range(0, 5);

    let rounding =
      rng.gen_range(-100.0f64, 8.0).max(0.0) * rng.gen_range(0.1, 1.0);
    ...
    if rng.gen_bool(if j == count - 1 { 0.2 } else { 0.9 })
      && !(objects.len() == 0 && j == count - 1)
    {
      continue;
    }
    // place animals
    let splits =
      (1.0 + rng.gen_range(0.0, 8.0) * rng.gen_range(0.0, 1.)) as usize;
    let extrapad = pad + 10.0;
    let scale_base = 4.0 + rng.gen_range(0.0, 5.0) * rng.gen_range(0.0, 1.0);

    let seed = rng.gen_range(0.0, 10000000.0);

    let offset = rng.gen_range(0.2, 0.8);

    let mut positions = Vec::new();
    for s in 0..splits {
      let p = (offset + s as f64) / (splits as f64);
      let x = extrapad + p * (width - 2.0 * extrapad);
      positions.push(x);
    }
    let split = if positions.len() == 1 {
      0
    } else {
      rng.gen_range(0, positions.len() - 1)
    };

    let objs: Vec<Vec<(usize, Vec<(f64, f64)>)>> = (0..splits)
      .into_par_iter()
      .map(|s| {
        let mut rng = rng_from_seed(seed + (s as f64) / 0.0234);
        let p = (offset + s as f64) / (splits as f64);
        let x = extrapad + p * (width - 2.0 * extrapad);
        let xi = ((x - mountainpadding) / precision) as usize;
        let v = smooth_heights[xi];
        let scale = scale_base * rng.gen_range(0.9, 1.1);
        let rot = s > split;
        if strictly_in_boundaries(
          (v.0, v.1),
          (extrapad, pad, width - extrapad, height - pad),
        ) {
          return bad_dog((v.0, v.1), scale, rot, &mut rng);
        }
        return vec![];
      })
      .filter(|a| a.len() > 0)
    ...
    let xmid = if positions.len() == 1 {
      positions[0] + rng.gen_range(0.1, 0.2) * width
    } else {
      let i = split;
      mix(positions[i], positions[i + 1], rng.gen_range(0.4, 0.6))
    };
    let xi = ((xmid - mountainpadding) / precision) as usize;
    let v = smooth_heights[xi];
    let jump = rng.gen_range(1.0, 4.0) * rng.gen_range(0.1, 1.0) * scale_base;
    let p = (v.0, v.1 - jump);
    if !strictly_in_boundaries(
      p,
      (extrapad, pad, width - extrapad, height - pad - jump),
    ) {
      continue;
    }

    objects.push(fowl(p, scale_base, 0.0, jump, &mut rng, &mut passage));

    for obj in objs {
      for (_s, route) in obj.iter() {
        for &p in route.iter() {
          passage.count(p);
        }
      }
  ...
  routes = vec![routes, objects.concat()].concat();
  ...
  let f = rng.gen_range(0.005, 0.01);
  let optim = rng.gen_range(0, 4);
  let mut all = Vec::new();

  for i in 0..3 {
    let ppad = rng.gen_range(3.0, 8.0);
    let min = ppad + 1.0;
    let max = ppad + rng.gen_range(1.0, 3.0);
    ...
    let abase = 0.0;
    for c in circles {
      let ang = abase + 2.0 * perlin.get([f * c.x, f * c.y, 44. + 3.3 * seed]);
      all.push(bg_shape((c.x, c.y), c.r, ang, &mut rng));
    }
  }

  routes = vec![routes, all.concat()].concat();
