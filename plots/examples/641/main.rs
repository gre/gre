...
pub struct Opts {
  ...
  pub height: f64,
  #[clap(short, long, default_value = "9.0")]
  pub seed: f64,
...
fn art(opts: &Opts) -> Vec<Group> {
  let colors = vec!["Teal", "Crimson", "SteelBlue"];
  let scales_per_color = vec![
    vec![0.1, 0.4, 0.8],
    vec![0.1, 0.4, 0.8],
    vec![0.1, 0.4, 0.8],
  ];
  ...
  let aoffset = rng.gen_range(0.0, 1.0);
  let sample_offset = rng.gen_range(-0.2, 0.6) * rng.gen_range(0.0, 1.0);
  let sample_pow = rng.gen_range(1.0, 4.0);

  let noiseamp = rng.gen_range(0.0, 1.0);
  let noisefreq = 0.6 + rng.gen_range(0.5, 9.0) * rng.gen_range(0.0, 1.0);

  let max_perturbation =
    (rng.gen_range(-100.0, 50.0) * rng.gen_range(0.0, 1f64)).max(0.0);

  let voronoi_size = 5 + rng.gen_range(0f64, 1000.0) as usize;

  let candidates = sample_2d_candidates_f64(
    &|p| sample_offset + (1.2 * euclidian_dist((0.5, 0.5), p)).powf(sample_pow),
    1000,
    voronoi_size,
    &mut rng,
  );
  ...
  let routes: Vec<(usize, Vec<Vec<(f64, f64)>>)> = polys
    ...
    .filter_map(|local_poly| {
      ...
      let threshold = noiseamp
        * perlin.get([
          opts.seed + noisefreq * perturbation,
          noisefreq * center.x(),
          noisefreq * center.y(),
        ]);
      let a = colors.len() as f64
        * ((threshold
          + aoffset
          + (2.0 * PI + (0.5 - center.y()).atan2(0.5 - center.x()))
            / (2.0 * PI))
          % 1.0);

      let color = (a as usize) % (colors.len());
