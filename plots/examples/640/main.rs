...
fn art(opts: &Opts) -> Vec<Group> {
  let colors = vec!["#FF9900", "#2200CC"];
  let scales_per_color = vec![vec![0.1, 0.4, 0.8], vec![0.1, 0.4, 0.8]];
  ...
  let max_perturbation =
    (rng.gen_range(-100.0, 100.0) * rng.gen_range(0.0, 1f64)).max(0.0);

  let voronoi_size =
    5 + (rng.gen_range(0f64, 1000.0) * rng.gen_range(0.0, 1.0)) as usize;
