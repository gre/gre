...
for page in $(seq 0 1 9); do
    cargo run --release --example wip -- --seed $seed --page $page --file results/svgs/seed_${seed}_${page}.svg &
done
wait
...
ffmpeg -y -r 12 -i tmp/seed_${seed}_%d.png -c:v libx264 -vf "pad=ceil(iw/2)*2:ceil(ih/2)*2,fps=8,format=yuv420p" videos/$seed.mp4
