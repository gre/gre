...
#[clap()]
struct Opts {
    #[clap(short, long, default_value = "677.0")]
    seed: f64,
}
...
fn waves_in_circle(
    ...
    loop {
        if base_y < circle.y + 0.2 * circle.r {
            break;
        }
        let precision = 0.2;
        ...
        loop {
            ...
            let y = base_y + (circle.r - euclidian_dist((circle.x, circle.y), (x, base_y))) * (
                0.5 * perlin.get([
                    0.01 * x,
                    0.02 * base_y,
                    seed + 2.0 * perlin.get([
                        0.03 * base_y + 0.01 * perlin.get([
                            0.8 * base_y,
                            0.8 * x,
                            100. + 7.3 * seed
                        ]),
...
fn waveball(n: usize, seed: f64, c: &VCircle) -> WaveballRes {
    ...
    let (waves, height_map) = waves_in_circle(seed, c, 0.3 + n as f64 * 0.2);
    
    let res = packing(seed, 200000, 50, 4.0 / (1. + n as f64), c, &height_map, 14.0 / (1. + 4. * n as f64), 100.0)
        .par_iter()
