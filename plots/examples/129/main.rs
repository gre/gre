...
fn art(opts: Opts) -> Vec<Group> {
  ...
  let size = 120.0;
  let project =
    |p: (f64, f64)| ((w - size) / 2. + p.0 * size, 10.0 + p.1 * size);
  let project_poly = |p: geo::Polygon<f64>| p.map_coords(|&c| project(c));
    ...
    .map(|(i, &color)| {
      ...
      if i == opts.technique % 2 {
        ...
        let pattern = |data, p: (f64, f64)| {
          let mut d = data;
          d = render_route(d, vec![(p.0 - 1., p.1), (p.0 + 1., p.1)]);
          d = render_route(d, vec![(p.0, p.1 - 1.), (p.0, p.1 + 1.)]);
          d
        };
        let count = 30;
        for y in 0..count {
          for x in 0..(if y % 2 == 1 { count - 1 } else { count }) {
            let p = (
              (0.5 + x as f64 + (y as f64 % 2.) * 0.5) / (count as f64),
              (0.5 + y as f64) / (count as f64),
            );
            let v = f(p);
            if v == 0.0 {
        ...
        l = l.add(signature(1.0, project((0.78, 1.0)), color));
      }
      match opts.technique % 8 {
        0 | 1 | 7 => {
          // vector field
          let candidates: Vec<(f64, f64)> =
            sample_2d_candidates_f64(&f, 400, 700, &mut rng);

          let mut passage = Passage2DCounter::new(0.005, 1.0, 1.0);

          let perlin = Perlin::new();

          let get_angle = |p: (f64, f64), i| {
            let mut delta = 0.;
            delta += PI * (if i % 2 == 0 { 1. } else { 0. });
            if opts.technique == 1 {
              delta -= PI / 2.;
            }
            delta
              + 4.
                * perlin.get([0.8 * p.0, 0.8 * p.1, 1. + opts.technique as f64])
              + (0.8 + opts.technique as f64 * 0.3)
                * perlin.get([2. * p.0, 2. * p.1, 10.])
              + 0.1 * perlin.get([200. * p.0, 200. * p.1, i as f64 / 100.0])
          };

          let initial_positions = candidates.clone();
          ...
          let mut build_route = |p, i, j| {
            ...
            if let Some(edge_p) =
              collide_segment_boundaries(p, nextp, (0.0, 0.0, 1.0, 1.0))
            {
              return Some((edge_p, true));
            }
            if f(nextp) < rng.gen_range(0., 0.002) {
              return None;
            }
          ...
          let routes = build_routes(initial_positions, &mut build_route);

          let mut data = Data::new();
          for route in routes {
            if route.len() > 4 {
              data =
                render_route(data, route.iter().map(|&p| project(p)).collect());
            }
          }
        ...
        2 => {
          // circles
          for c in sample_2d_candidates_f64(&f, 500, 2000, &mut rng) {
            let (x, y) = project(c);
            l = l.add(
              Circle::new()
                .set("cx", x)
        ...
        3 => {
          // spiral fill
          let candidates = sample_2d_candidates_f64(&f, 400, 300, &mut rng);
          ...
          polys.retain(|poly| {
            ...
            strictly_in_boundaries(centroid.unwrap().x_y(), (0., 0., 1., 1.))
              && poly_bounding_square_edge(poly) < 0.4
          });
          ...
          for poly in polys {
            ...
            let map_p = |(lx, ly)| (min.x + width * lx, min.y + height * ly);
            let mut candidates = sample_2d_candidates_f64(
              &|p| {
                let ap = map_p(p);
                if poly.contains(&geo::Point::new(ap.0, ap.1)) {
                  0.2 * f(ap)
                } else {
                  0.0
                }
            ...
            candidates =
              candidates.iter().map(|&p| project(map_p(p))).collect();
            data = render_route(data, route_spiral(candidates));
          }
        ...
        4 => {
          ...
          let candidates = sample_2d_candidates_f64(&f, 800, 320, &mut rng);
          ...
          polys.retain(|poly| {
            ...
            strictly_in_boundaries(centroid.unwrap().x_y(), (0., 0., 1., 1.))
              && poly_bounding_square_edge(poly) < 0.3
          });
          ...
          for poly in polys {
            ...
            let mul = if i == 0 { 1. } else { -1. };

            if candidates.len() > 5 {
              candidates.sort_by(|&a, &b| {
                (a.0 - mul * a.1)
                  .partial_cmp(&(b.0 - mul * b.1))
                  .unwrap()
                  .then(a.1.partial_cmp(&b.1).unwrap())
              });
              data = render_route(data, candidates);
            }
          }
        ...
        5 => {
          // voronoi
          let candidates = sample_2d_candidates_f64(&f, 400, 2800, &mut rng);
          ...
          polys.retain(|poly| {
            ...
            poly_bounding_square_edge(poly) < 0.1
              && strictly_in_boundaries(
                centroid.unwrap().x_y(),
                (0., 0., 1., 1.),
              )
              && poly.exterior().points_iter().all(|p| f(p.x_y()) > 0.001)
          });

          let mut data = Data::new();
          for poly in polys {
            data = render_polygon_stroke(data, project_poly(poly));
          }
        ...
        6 => {
          // voronoi + tsp
          let candidates = sample_2d_candidates_f64(&f, 400, 100, &mut rng);
          ...
          polys.retain(|poly| {
            ...
            strictly_in_boundaries(centroid.unwrap().x_y(), (0., 0., 1., 1.))
              && poly_bounding_square_edge(poly) < 0.5
          });

          let clouds: Vec<Vec<(f64, f64)>> = polys
            .iter()
            .map(|poly| {
              let bounds = poly.bounding_rect().unwrap();
              let min = bounds.min();
          ...
          let routes: Vec<Vec<(f64, f64)>> = clouds
            .into_par_iter()
            .map(|pts| {
              let mut route = tsp(pts, time::Duration::seconds(2));
              route.push(route[0]);
              route
            })
            .collect();
...
fn jumping_blob(f: f64, o: (f64, f64)) -> Vec<f64> {
  ...
  let v =
    smoothstep(-0.6, 0.0, s).powf(2.0) * (if s < 0.0 { 1.0 } else { 0.0 });
  vec![
