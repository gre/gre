...
fn art(opts: &Opts) -> svg::Document {
  let skulls_and_roses = Ink("Skulls and Roses", "#05206B", "#2E0033", 0.36);
  let crimson = Ink("Crimson", "#b24348", "#8b3343", 0.36);

  let paper = Paper("White", "#fff", false);
  let inks = vec![(skulls_and_roses, 3.0, 0.3), (crimson, 3.0, 0.35)];
  ...
  for (ink_i, (_ink, density, weight_map_prec)) in inks.iter().enumerate() {
    ...
    map.fill_fn(&mut rng, &mut |(x, y): (f64, f64), rng| {
      ...
      let yrel = y / height;
      let mul = mix(1.0, if ink_i == 0 { 1.0 - yrel } else { yrel }, 0.8);

      (value * density).min(max_density) * mul
    });
