...
fn art(opts: Opts) -> Vec<Group> {
    ...
    colors.iter().enumerate().map(|(ci, &color)| {    
        ...
        let radius = 80.;
        let in_circle = |p| {
            euclidian_dist(p, (width/2., height/2.)) < radius
        };
        let mut circle: Vec<(f64,f64)> = (0..csamples).map(|i| {
            let a = PI * 2. * i as f64 / csamples as f64;
            (width/2. + radius * a.cos(), height/2. + radius * a.sin())
        }).collect();
