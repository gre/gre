...
fn cell(
  ...
  let precision = 0.2;
  let count =
    (3.0 + rng.gen_range(5.0, 10.0) * rng.gen_range(0.2, 1.0)) as usize;
  ...
  for j in 0..count {
    ...
    let yincr = 0.4 + rng.gen_range(0.0, 2.5) * rng.gen_range(0.0, 1.0);
    ...
    let radius1 = rng.gen_range(0.5, 2.5);
    let radius2 = rng.gen_range(0.5, 2.5);
    let radius3 = rng.gen_range(0.5, 2.5);
    let radius4 = rng.gen_range(0.5, 2.5);
...
fn art(opts: &Opts) -> Vec<Group> {
  ...
  let colors = vec!["#105", sun_color, "#bbb"];
  colors
    .iter()
    .enumerate()
    .map(|(ci, color)| {
