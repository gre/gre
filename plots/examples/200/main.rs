...
fn waves_in_circle(
  ...
  let get_color = image_get_color("images/coffeecup2.jpg").unwrap();
  let f = |(x, y): (f64, f64)| {
    let p = (x, y);
    let c = get_color(p);
    smoothstep(0.53, 1.0, grayscale(c))
  };
          ...
          seed
            + 1.
              * perlin.get([
                0.3 * y,
                0.1 * x
                  + 0.1 * perlin.get([0.5 * y, 0.2 * x, 100. + 7.3 * seed]),
                10. + 0.3 * seed,
              ]),
        ]);
      y += 0.2 * (1. - l) * perlin.get([seed, 0.03 * x, 0.02 * base_y]);
...
fn art(opts: Opts) -> Vec<Group> {
  ...
  let colors = vec!["firebrick", "firebrick"];
