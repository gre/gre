...
fn shape (progress: f64, seed: f64, inside: bool) -> Path {
    ...
    let f = |origin: (f64, f64)| {
        ...
        let n2 = perlin.get([3. * p2.0, 3. * p2.1, 1.7 * seed + 1.5 * perlin.get([7. * p2.0, 7. * p2.1, 1.7 * seed])]);
...
fn art(opts: &Opts) -> Vec<Group> {
    ...
    let seed = opts.seed;
    vec![
        Group::new().set("transform", "translate(20,60)").add(shape(p1, seed, false)),
        Group::new().set("transform", "translate(160,60)").add(shape(p2, seed, true)),
        layer("black").add(
            signature(0.8, (120.0, 180.0), "black")
        )
    ]
}
