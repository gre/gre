...
fn art(opts: Opts) -> Vec<Group> {
  ...
  colors
    ...
    .map(|(ci, color)| {
      ...
      for (width, height, dx, dy, progress) in (0..div).map(|i| {
        let fi = i as f64;
        (
          w,
          h,
          20. * (cfi - 0.5) + 1.4 * cfi * (fi / fdiv - 0.5),
          (0.25 + 0.5 * cfi) * h + 0.8 * cfi * (fi / fdiv - 0.5) + fi * h,
          ((if ci == 0 { fi } else { fdiv - fi - 1. }) + 1.) / (fdiv + 2.),
        )
      }) {
        let maxr = height * 0.55;
        let dxp = 0.2;
