...
fn rec<R: Rng>(
  ...
  cut = cut
    ...
    .map(|p| {
      ...
      let amp = depth as f64 + 3.0;
      let poly = p.translate(amp * dx / dist, amp * dy / dist);
      poly
    })
    .collect();

  if depth <= 0 || rng.gen_range(0.0, 1.0) < 0.3 {
    return cut;
  }
  let mut all = Vec::new();
  for poly in cut {
...
fn art(opts: &Opts) -> Vec<Group> {
  ...
  let x1 = (width - size1) / 2.0;
  let x2 = (width + size1) / 2.0;
  let y1 = (height - size2) / 2.0;
  let y2 = (height + size2) / 2.0;
