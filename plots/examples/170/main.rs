...
struct Opts {
    ...
    seed: f64,
    #[clap(short, long, default_value = "66")]
    samples: usize,
}
...
fn art(opts: Opts) -> Vec<Group> {
    ...
    colors.iter().enumerate().map(|(ci, &color)| {  
        let f = |(x, y): (f64, f64)| {
            ...
            let f3 = 2.0;
            let f4 = 50.;
            let a1 = 0.5 * (2. * (0.65 - length(c)).max(0.).powf(1.3));
            let a2 = 1.4;
            let a3 = 1.6;
            let a4 = 0.008;
            let n1 = a1 * perlin.get([
                ...
                3.134 * opts.seed + a2 * perlin.get([
                    ...
                    opts.seed * 7.62 + 10. + a3 * perlin.get([
                        1.23456 * opts.seed + ci as f64 * 0.04 + a4 * perlin.get([ f4 * c.1, f4 * c.0 ]),
                        f3 * c.0,
                        f3 * c.1,  
                    ]),
                ])
        ...
        let thresholds: Vec<f64> = 
            (0..samples)
            .map(|i| {
                let k = (i as f64 + 0.4 * (ci as f64)) / (samples as f64);
