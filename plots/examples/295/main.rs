    ...
    colors.iter().enumerate().map(|(ci, &color)| {
        let f = |(x, y): (f64, f64)| {
            ...
            for _i in 0..res {
              ...
              p.1 += rng.gen_range(-0.4, 0.4) * rng.gen_range(0.0, 1.0);
            ...
            let n = rng.gen_range(0.0, 1.0) * perlin.get([
                ...
                f1 * c.1,
                opts.seed
                + (rng.gen_range(-0.5, 0.5f64).max(0f64)) * (ci as f64)
                + a2 * perlin.get([
                    7. + opts.seed
                    + (rng.gen_range(-0.5, 0.5f64).max(0f64)) * (ci as f64),
        ...
        let thresholds: Vec<f64> = 
            (0..samples)
            .map(|i| 
                ((ci as f64) / (colors.len() as f64) + i as f64) / (samples as f64).floor())
            .collect();
