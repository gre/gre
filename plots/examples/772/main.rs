...
fn art(opts: &Opts) -> Vec<Group> {
  ...
  let get_color = image_get_color("images/shape2.png").unwrap();

  let in_shape =
    |p: (f64, f64)| -> bool { get_color((p.0 / width, p.1 / height)).0 < 0.5 };
  ...
  /*
  let (clr1, clr2) = if rng.gen_bool(0.2) {
    (5, 5)
  } else if rng.gen_bool(0.2) {
    (0, 1)
  } else {
    (rng.gen_range(0, clrs.len()), rng.gen_range(0, clrs.len()))
  };
  */
  ...
  let f = rng.gen_range(0.01, 2.0)
    ...
    * rng.gen_range(0.0, 1.0);
  let mult = rng.gen_range(0.1, 2.0);
  let offset = rng.gen_range(1.0, 5.0);
  for c in circles {
    let v = offset + mult * perlin.get([f * c.x, f * c.y, opts.seed]);
    let clr = ((clrs.len() - 1) as f64 * v) as usize % clrs.len();
    stats[clr] += c.r * c.r;
    let res = c.r as usize * 4 + 12;
    let dr = rng.gen_range(0.7, 1.3);
    //routes.push((clr, circle_route((c.x, c.y), c.r, res)));
    //routes.push((clr, spiral_optimized(c.x, c.y, c.r, dr, 0.2)));
  }

  let line1from = (850. / 3000.0, 710. / 3000.0);
  let line1to = (2330. / 3000.0, 2010. / 3000.0);
  let line2from = (1270. / 3000.0, 560. / 3000.0);
  let line2to = (2250. / 3000.0, 2171. / 3000.0);
  ...
  let clr1 = s[0].0;
  let clr2 = if s[1].1 / s[0].1 < 0.1 { clr1 } else { s[1].0 };

  for (fromp, top, ang, clr) in vec![
    (line1from, line1to, -1.7, clr1),
    (line2from, line2to, 1.3, clr2),
  ] {
    //let clr = rng.gen_range(0, clrs.len());
    let from = (fromp.0 * width, fromp.1 * height);
    let to = (top.0 * width, top.1 * height);
    let l = euclidian_dist(from, to);
    for i in 0..lines_count {
      let p = (i as f64 + 0.5) / (lines_count as f64) - 0.5;
      let dx = (to.0 - from.0) / l;
      let dy = (to.1 - from.1) / l;
      let (dx, dy) = p_r((dx, dy), ang);
      let a = (from.0 + dx * widthfrom * p, from.1 + dy * widthfrom * p);
      let b = (to.0 + dx * widthto * p, to.1 + dy * widthto * p);
      let l = euclidian_dist(a, b);
      let dx = (b.0 - a.0) / l;
      let dy = (b.1 - a.1) / l;
      let incr = 0.05;
      let mut v = 0.0;
      ...
      loop {
        p = (a.0 + dx * v, a.1 + dy * v);
        if !in_shape(p) && v > height * 0.4 {
          break;
        }
        v += incr;
      }

      routes.push((clr, vec![a, p]));
    }
  }
