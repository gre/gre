...
fn art(opts: &Opts) -> svg::Document {
  ...
  let mut ground_inks = vec![];

  let stability = rng.gen_range(0.3, 1.0);

  if rng.gen_bool(stability) {
    ground_inks.push((
  ...
  if rng.gen_bool(stability) {
    ground_inks.push((moonstone, 0.7));
  }

  if rng.gen_bool(0.5) {
    rng.shuffle(&mut ground_inks);
  }
  inks.extend(ground_inks);
  ...
  let max_density = 2.0;
  let distfactor = rng.gen_range(1.0, 3.0);
  let valueadd = rng.gen_range(0.5, 0.9);
  let amp = rng.gen_range(2.2, 3.0);
  let f1 = rng.gen_range(1.0, 2.5);
  let amp1 = rng.gen_range(0.5, 1.0);
  let f2 = rng.gen_range(4.0, 8.0);
  let amp2 = rng.gen_range(0.2, 0.6);
  let f3 = rng.gen_range(8.0, 36.0);
  let amp3 = rng.gen_range(0.1, 0.5);
  let f4 = rng.gen_range(4.0, 5.0);
  let halo = rng.gen_range(0.3, 0.5);
  let sea_randomness = rng.gen_range(0.4, 1.0);
  ...
  map.fill_fn(&mut rng, &mut |(x, y): (f64, f64), rng| {
    ...
    let mut value: f64 = rng.gen_range(0.0, 1.0)
      + 0.5
        * inksf
        * (valueadd - distfactor * distf + mix(n1, 0.0, distf.powf(2.0)));

    // mix the sea color a bit
    let threshold = 3.7;
    if value < threshold - halo {
      value = mix(value, rng.gen_range(0.0, threshold), sea_randomness);
    } else if value < threshold + halo {
      return (6, 0.0);
    }
  ...
  /*
  let prec = precision * 2.0;
  let f = |p: (f64, f64)| map.get_color((p.0 * width, p.1 * height)) as f64;
  let thresholds = vec![5.0];
  let w = (width / prec) as u32;
  let h = (height / prec) as u32;
  let res = contour(w, h, f, &thresholds);
  let contour_routes = crop_routes(
    &features_to_routes(res, prec),
    (pad, pad, width - pad, height - pad),
  );
  for r in contour_routes {
    routes.push((4, r));
  }
  */
