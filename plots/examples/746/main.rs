...
fn art(opts: &Opts) -> Vec<Group> {
  ...
  let overlap =
    |p| passage.get(p) == 0 && strictly_in_boundaries(p, extrabound);

  let does_overlap = |c: (f64, f64, f64)| {
    overlap((c.0, c.1))
      && circle_route((c.0, c.1), c.2, shape, circleang)
        .iter()
        .all(|&p| overlap(p))
  };

  let count = 1 + (rng.gen_range(0.0, 3.5) * rng.gen_range(0.0, 1.0)) as usize;
  for i in 0..count {
    ...
    for c in circles {
      routes.push((2, circle_route((c.x, c.y), c.r, shape, circleang)));
    }
  }
