...
fn art(opts: Opts) -> Vec<Group> {
    ...
    colors.iter().enumerate().map(|(ci, &color)| {
        let f = |(x, y): (f64, f64)| {
            ...
            let res = (1. + rng.gen_range(0.0, 60.0) * rng.gen_range(0.0, 1.0)) as usize;
            let mut s = 100f64;
            ...
            let a1 = rng.gen_range(0.0, 0.4) * rng.gen_range(0.0, 1.0) * rng.gen_range(0.0, 1.0);
            ...
            let n = a1 * perlin.get([
                ...
                f1 * c.1,
                opts.seed
                + (rng.gen_range(-0.1, 0.2f64).max(0f64)) * rng.gen_range(0.0, 1.0) * (ci as f64)
                + a2 * perlin.get([
                    7. + opts.seed,
                    f2 * c.0 + a3 * perlin.get([f3 * c.0, f3 * c.1, 1. + opts.seed]) + rng.gen_range(-0.1, 0.2f64).max(0f64) * rng.gen_range(0.0, 1.0) * (ci as f64),
        ...
        let thresholds: Vec<f64> = 
            (0..samples)
            .map(|i| 
                ((ci + i) as f64 + pattern.1 * (i as f64 / pattern.0).floor()) / (samples as f64 * (pattern.0+pattern.1) / pattern.0).floor())
            .collect();
