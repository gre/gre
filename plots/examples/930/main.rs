...
pub struct Opts {
  ...
  pub pad: f64,
  #[clap(short, long, default_value = "3.9")]
  pub gridsize: f64,
  #[clap(short, long, default_value = "3.0")]
  pub fontsize: f64,
  #[clap(short, long, default_value = "gm")]
  pub text: String,
...
fn generate_maze<R: Rng>(
  ...
  while let Some(current) = stack.pop_back() {
    ...
    for neighbor in neighbors {
      if !is_inside(neighbor.1, neighbor.0) {
        continue;
      }
...
fn art(opts: &Opts) -> Vec<Group> {
  ...
  let get_image = image_get_color("images/gm.png").unwrap();

  let f = |x: usize, y: usize| -> bool {
    let xp = x as f64 / ((w + 1) as f64);
    let yp = y as f64 / ((h + 1) as f64);
    get_image((xp, yp)).0 < 0.5
  };

  let initial_samples = 2;

  let lines = generate_maze(&mut rng, initial_samples, &f, w, h);
