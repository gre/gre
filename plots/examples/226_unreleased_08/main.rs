    ...
    colors
        ...
        .map(|(ci, color)| {
            ...
            let f = |p: (f64, f64)| {
                ...
                let b1 = noise.get([ p.0 + k * 3. * a1 + 4.8 + opts.seed, p.1 + k * 2. * a2 - 3.7 ]);
                let b2 = noise.get([ p.0 + k * 2. * a1 + 7.8 - opts.seed, p.1 + k * 3. * a2 - 1.7 ]);
                let v = smoothstep(
                    -0.2,
                    0.5,
                    noise.get([-77.777 * opts.seed, k * p.0 + a2 + 2. * b1, k * p.1 + a1 + 2. * b2])
                );
                if ci == 0 {
                    v
                }
