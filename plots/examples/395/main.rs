...
fn packing(
  ...
  let mut circles = initial_circles;
...
fn art(opts: Opts) -> Vec<Group> {
  ...
  let max_passage = 8;
  let centercr = 120.0;
  let centercircle = VCircle::new(width / 2.0, height / 2.0, centercr);
  let mut primaries = packing(
    opts.seed,
    500000,
    2000,
    4,
  ...
  for (color, from, to) in themes {
              ...
              + rng.gen_range(0.0, 1.0)
                * perlin.get([
                  ...
                  10.
                    + seed
                    + rng.gen_range(0.0, 1.0)
                      * perlin.get([0.2 * g.0, 0.2 * g.0, seed]),
                ]),
          ]);
    ...
    let inside = |from, to| {
      strictly_in_boundaries(from, bounds) && strictly_in_boundaries(to, bounds)
    };
