...
struct Opts {
  ...
  file: String,
  #[clap(short, long, default_value = "118.0")]
  seed: f64,
...
fn art(opts: &Opts) -> Vec<Group> {
  ...
  let colors = vec![("#2c5881", 20000), ("#a89867", 15000), ("#f22", 10000)];
  ...
  let ampang = rng.gen_range(1.0, 2.0);
  let f1 = rng.gen_range(0.5, 2.0) * rng.gen_range(0.0, 1.0);
  let f2 = rng.gen_range(0.5, 2.0) * rng.gen_range(0.0, 1.0);
  ...
  for (ci, &(color, particles)) in colors.iter().enumerate() {
    ...
    let mut l = layer(format!("{} {}", ci, color).as_str());
