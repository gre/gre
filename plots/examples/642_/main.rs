...
fn art(opts: &Opts) -> Vec<Group> {
  let colors = vec!["RoyalBlue", "Tomato"];
  ...
  let sample_offset = rng.gen_range(-0.1, 0.5);
  let sample_pow = rng.gen_range(1.0, 4.0);

  let modmul = rng.gen_range(3.0, 20.0);
  ...
  let voronoi_size = rng.gen_range(10.0, 600.0) as usize;

  let candidates = sample_2d_candidates_f64(
    &|p| sample_offset + (2f64 * (p.0 - 0.5).abs()).powf(sample_pow),
    1000,
    voronoi_size,
    &mut rng,
  );
  ...
  let routes: Vec<(usize, Vec<Vec<(f64, f64)>>)> = polys
    ...
    .filter_map(|local_poly| {
      ...
      let a = aoffset + center.y() * modmul + threshold;
