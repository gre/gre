...
fn art(opts: &Opts) -> Vec<Group> {
  ...
  for _j in 0..count {
    ...
    let ynoisefactor = rng.gen_range(0.0, 0.2);
    let yincr = 0.5
      ...
      .max(0.0)
      .min(2.0);
    let amp1 = rng.gen_range(0.0, 8.0);
    let amp2 = rng.gen_range(0.0, 2.0);
    let amp3 = rng.gen_range(0.0, 1.0);
    let offsetstrategy = rng.gen_range(0, 5);
    let center = rng.gen_range(0.2, 0.8) * width;

    let stopy = height * rng.gen_range(-0.1, 0.8);

    // Build the mountains bottom-up, with bunch of perlin noises
    let mut base_y = height * 3.0;
    let mut miny = height;
    loop {
      if miny < stopy {
        break;
  ...
  let split = 0.0;
  let slide_effect = rng.gen_range(2.0, 8.0);
  for i in 0..count {
  ...
  for (c, r) in routes_copy {
    ...
    if l > 0 {
      ...
      let mut all = vec![];
      for p in r.clone() {
        if strictly_in_boundaries(p, bound) && passage.count(p) < 10 {
          route.push(p);
        } else {
          let l = route.len();
          if l > 1 {
