...
fn art(opts: &Opts) -> Vec<Group> {
  ...
  let f = |(x, y)| {
    let mut p = (mix(-0.0, 1.0, x), mix(-0.15, 1.15, y));
    let freq = 20.0;
    let amp = 0.05;
    p.0 += amp * perlin.get([0.7 + 1.2 * opts.seed, freq * x, freq * y]);
    p.1 += amp * perlin.get([-7.3 * opts.seed, freq * x, freq * y]);
    let freq = 40.0;
    let amp = 0.02;
    p.0 += amp * perlin.get([36.7 + 0.7 * opts.seed, freq * x, freq * y]);
    p.1 += amp * perlin.get([7.7 - 0.3 * opts.seed, freq * x, freq * y]);
    let freq = 80.0;
    let amp = 0.005;
    p.0 += amp * perlin.get([36.7 + 0.7 * opts.seed, freq * x, freq * y]);
    p.1 += amp * perlin.get([7.7 - 0.3 * opts.seed, freq * x, freq * y]);
  ...
  for center in vec![(width / 2.0, height / 2.0), (-width * 0.5, height * 0.5)]
  {
    let aincr = 0.001;
    let rincr = aincr / 6.0;
    let mut r = 0.1;
