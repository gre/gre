...
fn art(opts: Opts) -> Vec<Group> {
  ...
  colors
    ...
    .map(|(i, color)| {
      ...
      let project = |(x, y): (f64, f64)| (pad + x * width, pad + y * height);
      ...
      for i in 0..1 {
        let candidates = sample_2d_candidates_f64(
          ...
          800,
          (voronoi_size as f64 * rng.gen_range(0.1, 1.2)) as usize,
          &mut rng,
        );

        let mut polys = sample_square_voronoi_polys(candidates, 0.1);
        ...
        let get = |p| 0.45 - euclidian_dist(p, (0.5, 0.5));
