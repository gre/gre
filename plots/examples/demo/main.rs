...
fn art(opts: &Opts) -> svg::Document {
  ...
  let skulls_and_roses = Ink("Skull and Roses", "#05206B", "#2E0033", 0.35);
  let paper = Paper("White", "#fff", false);
  let inks = vec![
    (poppy_red, 2.0, 0.3),
    (fire_and_ice, 2.0, 0.4),
    (skulls_and_roses, 2.0, 0.5),
  ];
  ...
  let f1 = rng.gen_range(6.0, 14.0);
  let amp1 = rng.gen_range(0.5, 3.0);
  let f2 = f1 * rng.gen_range(2.0, 4.0);
  let amp2 = rng.gen_range(2.0, 10.0);
  let f3 = f2 * rng.gen_range(2.0, 4.0);
  ...
  let f7 = rng.gen_range(0.0, 20.0);
  let amp7 = rng.gen_range(0.5, 2.0);
  let f8 = f7 * rng.gen_range(1.0, 3.0);
  let amp8 = rng.gen_range(0.0, 2.0);
  let f9 = f8 * rng.gen_range(1.0, 3.0);
  let amp9 = rng.gen_range(0.0, 2.0) * rng.gen_range(0.0, 1.0);
  let f10 = rng.gen_range(0.0, 2.0);
  let div1 = rng.gen_range(10.0, 30.0);
  let div2 = rng.gen_range(40.0, 80.0);
  let div3 = rng.gen_range(150.0, 200.0);
  let r = rng.gen_range(8.0, 150.0);
  let r2 = rng.gen_range(20.0, 100.0);
  let w = rng.gen_range(0.0, 20.0);
  let ampbase = rng.gen_range(0.0, 10.0);
  ...
  for (ink_i, (_ink, density, weight_map_prec)) in inks.iter().enumerate() {
    ...
    map.fill_fn(&mut rng, &mut |(x, y): (f64, f64), rng| {
      ...
      let blur = smoothstep(0.5 * width + pad, pad, (width - x).min(x));
      ...
      let amp = ampbase
        * 2.0
        * perlin
          .get([opts.seed / 0.136, f10 * x / width, f10 * y / width])
          .max(0.0);
      let ang = PI * amp7 * n2;
      let x = x + amp * ang.cos();
      let y = y + amp * ang.sin();
    ...
    while x < width {
      let mut y = 0.0;
      while y < height {
        let v = map.get_weight((x, y)) / max_density;
        if v < 0.1 {
          y += step;
          continue;
        }
        let ld = mix(step, 0.3, v - 0.1);
        let mut l = rng.gen_range(-0.1, 0.1);
        while l < step {
          routes.push((ink_i, vec![(x, y + l), (x + step, y + l)]));
          l += ld;
        }
        y += step;
      }
...
pub struct Opts {
  ...
  pub pad: f64,
  #[clap(short, long, default_value = "77.0")]
  pub seed: f64,
}
