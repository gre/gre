...
fn art(opts: &Opts) -> Vec<Group> {
  ...
  let minpad = 10.0;
  let aw = width - 2.0 * minpad;
  let ah = height - 2.0 * minpad;
  let caw = aw / (W as f64);
  let cah = ah / (H as f64);
  let ca = caw.min(cah);
  let padx = (width - ca * W as f64) / 2.0;
  let pady = (height - ca * H as f64) / 2.0;

  let w = (width - 2. * padx) / (W as f64);
  let h = (height - 2. * pady) / (H as f64);
  for xi in 0..W + 1 {
    routes.push((
      0,
      vec![
        (padx + xi as f64 * w, pady),
        (padx + xi as f64 * w, pady + H as f64 * h),
      ],
    ));
  }
  for yi in 0..H + 1 {
    routes.push((
      0,
      vec![
        (padx, pady + yi as f64 * h),
        (padx + W as f64 * w, pady + yi as f64 * h),
      ],
    ));
  }
  ...
  for i in 0..frames {
    let progress = i as f64 / (frames as f64 - 1.);
    let yi = i / W;
    let xi = i % W;
    let x = padx + xi as f64 * w;
    let y = pady + yi as f64 * h;

    let bound = (x, y, x + w, y + h);

    let get_color =
      image_gif_get_color("images/YoungGrossHoopoe.gif", i % gif_frames)
        .unwrap();

    routes.extend(filling.fill(
      &mut rng,
      &|x, y| {
        let p = normalize_in_boundaries((x, y), bound);
        let mut q = p;
        q.0 = q.0 * 2.0 - 0.5;
        q.1 = q.1 * 2.0 - 0.5;
        let c = get_color(q);
        if strictly_in_boundaries(q, (0.0, 0.0, 1.0, 1.0)) && grayscale(c) < 0.5
        {
          density
        } else {
          let d = euclidian_dist(p, (0.5, 0.5));
          let a = (2.5 * PI + (p.1 - 0.5).atan2(p.0 - 0.5)) % (2.0 * PI);
          if r1 < d && d < r2 {
            if a < progress * PI * 2.0 {
              density
            } else {
              1.5
            }
