...
fn waves_in_circle(
  ...
  let get_color_2 =
    image_gif_get_color("images/eye.gif", opts.index + 2).unwrap();
  let f = |(x, y): (f64, f64)| {
    let p = (x, y);
    let c1 = get_color_1((p.0, p.1 + 0.2));
    let c2 = get_color_2((p.0, 1.2 - p.1));
    smoothstep(0.0, 1.0, grayscale(c1)) * smoothstep(0.0, 1.0, grayscale(c2))
  };
          ...
          seed
            + 1.2
              * perlin.get([
                0.8 * y,
                0.1 * x
                  + 0.1 * perlin.get([1.0 * y, 0.2 * x, 100. + 7.3 * seed]),
                10. + 0.3 * seed,
              ]),
        ]);
      y += 2.0 * (1. - l) * perlin.get([seed, 0.03 * x, 0.02 * base_y]);
