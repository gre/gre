...
fn art(opts: Opts) -> Vec<Group> {
  ...
  let colors = vec!["red", "red", "red", "red"];
  let dim = (297., 210.);
  let pad = 10.0;
  ...
  let initial: Vec<(usize, (f64, f64))> = colors
    ...
    .flat_map(|(ci, _color)| {
      ...
      let r =
        10.0 + 100.0 * ((ci as f64) / (colors.len() as f64 - 1.0)).powf(2.0);
      let res = (20.0 + r / spacing) as usize;
      for j in 0..3 {
        let a = (0.25 + j as f64 + ci as f64 * 0.5) * PI * 2. / 3.;
  ...
  let initial_a: Vec<f64> = initial_positions
    .iter()
    .map(|p| (p.1 - c.1).atan2(p.0 - c.0) + initial_angle_off)
    .collect();
  ...
  colors
    ...
    .map(|(i, color)| {
      ...
      data = render_route(
        ...
        initial
          .iter()
          .filter_map(|&(ci, origin)| if ci == i { Some(origin) } else { None })
          .collect(),
      );
      ...
      if i == colors.len() - 1 {
        l = l.add(signature(1.0, (180.0, 180.0), color));
      }
      l
    })
    .collect()
...
#[clap()]
struct Opts {
  #[clap(short, long, default_value = "32.0")]
  seed: f64,
}
