...
#[clap()]
struct Opts {
  #[clap(short, long, default_value = "112.0")]
  seed: f64,
...
fn art(opts: &Opts) -> Vec<Group> {
  ...
  let cpad = rng.gen_range(2.0, 3.0);
  let min_scale = cpad + rng.gen_range(1.0, 2.0);
  let max_scale = min_scale + rng.gen_range(2.0, 4.0);

  let xpadlayer = 10.0;
  let count = 3;

  let colors = vec!["#Fb2", "#F3d", "#0FC", "#09F"];
  ...
  colors
    ...
    .map(|(i, color)| {
      ...
      for j in 0..count {
        ...
        for c in circles {
          let res = c.r as usize * 3 + 10;
          routes.push(circle_route((c.x, c.y), c.r, res));
          routes.push(spiral_optimized(c.x, c.y, c.r, 1.0, 0.3));
        }
      }
