...
fn art(opts: Opts) -> Vec<Group> {
    ...
    let f = |(x, y): (f64, f64)| {
        let mut rng = rng_from_seed(10000. + 1.738 * opts.seed);
        let mut c = ((x-0.5) * width / height, y-0.5);
        if rng.gen_range(0.0, 1.0) < 0.6 {
            c.1 = c.1.abs();
        }
        let res = (1. + rng.gen_range(0.0, 24.0) * rng.gen_range(0.0, 1.0)) as usize;
        let mut s = 100f64;
        ...
        let a1 = rng.gen_range(0.0, 0.15) * rng.gen_range(0.0, 1.0);
    ...
    colors.iter().enumerate().map(|(ci, &color)| {
        ...
        for route in routes.clone() {
            let r = route.iter().map(|&p| {
                let x = if ci == 0 { width - p.0 } else { p.0 } + delta * (ci as f64 - 0.5);
                (x, p.1)
            }).collect();
