...
fn art(opts: &Opts) -> Vec<Group> {
    let colors = vec!["HotPink", "LightSteelBlue"];
    let seed = opts.seed;
    ...
    let target_size = rng.gen_range(20, 100);
    ...
    let max_passage = 6;
    let gridw = (1.
        + rng.gen_range(0., 100.) * rng.gen_range(0.0, 1.0))
        as i32;
    let gridh = (1.
        + rng.gen_range(0., 100.) * rng.gen_range(0.0, 1.0))
        as i32;
    let min_threshold = rng.gen_range(4.0, 32.0);
    let circle_bounds = (
        -0.2 * width,
        -0.2 * height,
        1.2 * width,
    ...
    for (ci, &color) in colors.iter().enumerate() {
        ...
        let routes: Vec<Vec<(f64, f64)>> = samples
            ...
            .map(|(si, &sample)| {
                let mut rng =
                    rng_from_seed(seed + si as f64 * PI);
                ...
                loop {
                    ...
                    for p in circles.iter() {
                        let dist =
                            euclidian_dist((p.x, p.y), g)
                                - p.r;
                        let a = (p.y - g.1)
                            .atan2(p.x - g.0)
                            + (si as f64 - 0.5) * PI;
                        let r =
                            smoothstep(100.0, -30.0, dist);
                        v.0 += r * a.cos();
                        v.1 += r * a.sin();
                    }
