...
// TODO letter to follow a general path => rotateCurve
// TODO scale differently the letters
// TODO allow "space" as a possible letter
// TODO more letters?
// modulate more?
// IDEA: zig zag of a known text
...
function art(S) {
  ...
  let scale = 0.05;
  let lineHeight = 250 * scale;
  let pad = 40;
  let curves = [];
  let y = pad + lineHeight / 2;
  let x = pad;
  let endofpage = false;
  let shaking = 0;
  const sentence = "this is not generative art";
  const newline = () => {
    x = pad;
    y += lineHeight;
  };
  while (!endofpage) {
    for (let i = 0; i < sentence.length; i++) {
      ...
      c = shakeCurve(rand, c, shaking);
      c = scaleCurve(c, scale);
      c = translateCurve(c, x, y);
      curves.push([[x, y], c]);
      x += w;
    }
    newline();
    shaking = shaking * 1.2 + 3;
  }
