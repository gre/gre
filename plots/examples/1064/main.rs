...
/**
 * @greweb 2023 – Pandemonium – participation to WAT 2023
 * Code is Art.
 */
fn art(opts: &Opts) -> svg::Document {
  ...
  let inks = vec![
    (writers_blood, 1.0, 0.4),
    (red_dragon, 2.5, 0.4),
    (fire_and_ice, 2.5, 0.4),
    (skulls_and_roses, 1.0, 0.4),
  ];

  let perlin = Perlin::new();
  let mut rng = rng_from_seed(opts.seed);
  let addvalue = rng.gen_range(0.5, 2.0);
  ...
  let div1 = rng.gen_range(0.0, 20.0);
  let div2 = rng.gen_range(0.0, 50.0);
  let div3 = rng.gen_range(0.0, 60.0);
...
pub struct Opts {
  ...
  pub pad: f64,
  #[clap(short, long, default_value = "37.0")]
  pub seed: f64,
}
