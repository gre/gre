...
fn art(opts: &Opts) -> Vec<Group> {
  ...
  for j in 0..count {
    ...
    let ampfactor = rng.gen_range(0.0, 0.2) * rng.gen_range(0.1, 1.0);
    let ynoisefactor = rng.gen_range(0.02, 0.2);
    let yincr = 0.3 + rng.gen_range(0.0, 5.0) * rng.gen_range(0.0, 1.0);
    ...
    // place animals
    let splits =
      (1.0 + rng.gen_range(0.0, 16.0) * rng.gen_range(0., 1.)) as usize;
    let extrapad = pad + 6.0;
    let scale_base = 5.0
      + rng.gen_range(0.0, 1.8) * rng.gen_range(0.0, 1.0)
      + 10.0 / (splits as f64 + 1.0);
    for s in 0..splits {
      ...
      {
        objects.push(object((v.0, v.1), scale, rot, &mut rng, &mut passage));
        // objects.push(vec![(0, vec![(0.0, 0.0)])]);
      }
    }
  }
  ...
  let ppad = rng.gen_range(2.0, 8.0);
  let min = ppad * 1.05 + rng.gen_range(1.5, 2.0);
  let max = min + rng.gen_range(0.2, 1.5);
  ...
  let abase = rng.gen_range(-PI, PI);
  let f = rng.gen_range(0.01, 0.2)
    * rng.gen_range(0.0, 1.0)
    * rng.gen_range(0.0, 1.0)
    * rng.gen_range(0.0, 1.0);
  let mut all = Vec::new();
  for c in circles {
    let ang = abase + 3.0 * perlin.get([f * c.x, f * c.y, 44. + 3.3 * seed]);
