...
fn art(opts: Opts) -> Vec<Group> {
    ...
    let mut seed = 8.8 + opts.seed * 7.7;
    let width = 420.0;
    let height = 297.0;
    let pad = 10.0;
    ...
    let max_c = min_c + rng.gen_range(1.0, 90.0);
    let secondary_threshold = rng.gen_range(20.0, 100.0);
    let packing_padding = 2.0 + rng.gen_range(0.0, 30.0) * rng.gen_range(0.0, 1.0);
    let primaries = packing(
        ...
        min_c,
        min_c + rng.gen_range(100.0, 400.0),
    );
    let mut circles = Vec::new();
    let mut index = 0;
    for c in primaries {
        if c.r > secondary_threshold {
            for c2 in packing(
                ...
                circles.push((index, c2));
            }
            seed = seed * 1.1 + 0.3;
        }
        else {
            circles.push((index, c));
        }
    ...
    for (ci, color) in colors.iter().enumerate() {
        ...
        let samples = sample_2d_candidates_f64(&|p| {
            ...
            let mut d = 99f64; 
            for &(index, circle) in circles.iter() {
                if ci == index {
        ...
        }, 1000, particles, &mut rng);

        let routes: Vec<Vec<(f64, f64)>> = samples.par_iter().enumerate().map(|(si, &sample)| {
            ...
            loop {
                ...
                ang += ampang * perlin.get([
                    f1 * 0.02 * g.0,
                    f1 * 0.02 * g.1,
                    2.4 * seed + rng.gen_range(1.0, 2.0) * perlin.get([
                        f2 * 0.04 * g.0,
                        f2 * 0.04 * g.1,
                        seed + rng.gen_range(1.0, 2.0) * perlin.get([
                            f3 * 0.1 * g.0,
                            seed,
                            f3 * 0.1 * g.1,
                        ])
