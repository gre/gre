...
fn rec_packing(
  ...
  let secondaries = primaries
    ...
    .filter(|&(i, p)| p.r > pad)
    .map(|(j, p)| {
      let mut rng = rng_from_seed(seed + j as f64 * 0.7);
      let newci = if rng.gen_bool((0.4 - i as f64 / 10.).max(0.)) {
        (ci + 1) % 2
      } else {
        ci
      };
      rec_packing(i + 1, newci, 7.7777 * p.x + 9.95731 * p.y + seed / 3., &p)
    })
    .collect::<Vec<_>>()
    .concat();

  let circles: Vec<(usize, VCircle)> =
    primaries.iter().map(|&c| (ci, c)).collect();
...
fn art(opts: Opts) -> Vec<Group> {
  ...
  let mut circles = rec_packing(0, 1, opts.seed, &bounds_container);
  circles.push((1, bounds_container));
