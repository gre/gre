...
fn art(opts: Opts) -> Vec<Group> {
  ...
  for (i, color) in colors.iter().enumerate() {
    ...
    let parametric = |t: f64| {
      ...
      let scale = 1.0 - t2;
      let s = spins;
      let dx = (2. * fp - 1.) * 0.3 * (((t).powf(2.0) - 0.3).abs() - 0.3);
      let dy = 0.0;
      let mut p = (
        dx + scale
          * amp1
      ...
      let noise_angle = 2. * PI * perlin.get([p.0, p.1, 1000.0 + fp]);
      let noise_amp = 0.01
        * (1.0 - t).powf(2.0)
        * (0.5 - p.1).max(0.0)
        * perlin
          .get([20.0 * p.0, 20.0 * p.1, -1000. + 10. * pingpong])
          .max(0.);
      p.0 += noise_amp * noise_angle.cos();
      p.1 += noise_amp * noise_angle.sin();
      // med freq noise
      let noise_angle =
        2. * PI * perlin.get([3. * p.0, 3. * p.1, 100.0 + 4. * fp]);
      let noise_amp = 0.01 * perlin.get([6. * p.0, 6. * p.1, 100. + 10. * fp]);
      p.0 += noise_amp * noise_angle.cos();
      p.1 += noise_amp * noise_angle.sin();
      // low freq noise
      let noise_angle = 2. * PI * perlin.get([0.6 * p.0, 0.6 * p.1, 50. + fp]);
      let noise_amp = 0.1 * perlin.get([1.2 * p.0, 1.2 * p.1, -50. + fp]);
      p.0 += noise_amp * noise_angle.cos();
      p.1 += noise_amp * noise_angle.sin();
      p
    };
    ...
    if i == 0 {
      l = l.add(signature(1.0, (160.0, 230.0), color));
    }
    layers.push(l);
  }
