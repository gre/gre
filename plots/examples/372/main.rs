...
fn draw_cell<R: Rng>(rng: &mut R, route: &Vec<(f64, f64)>, roof: &Vec<(f64, f64)>, ceil: &Vec<(f64, f64)>) -> Vec<Vec<(f64, f64)>> {
    ...
    let effective_height = (1.0 - 0.5 * rng.gen_range(0.0, 1.0) * rng.gen_range(0.0, 1.0)) * height;
    ...
    if effective_height > 2.0 && effective_width > 2.0 {
        ...
        let mut h = rng.gen_range(0.4, 0.9) * effective_height / (winy as f64 + 1.0);
        if winx > 1 && winy > 1 && w > 1.0 && h > 1.0 {
            ...
            for x in 1..(winx + 1) {
                if x % modx == 0 { continue };
                if (1 + x / modx) % modx2 == 0 { continue };
                let dx = (width - effective_width) / 2.0;
                let cx = mix(x1 + dx, x1 + dx + effective_width, (x as f64) / (winx as f64 + 1.));
                for y in 1..(winy+1) {
                    if y % mody == 0 { continue };
                    if (1 + y / mody) % mody2 == 0 { continue };
                    let cy = mix(y1, y1 + effective_height, (y as f64) / (winy as f64 + 1.));
                    let rect = vec![
                        (cx - w / 2.0, cy - h / 2.0),
                        (cx + w / 2.0, cy - h / 2.0),
                        (cx + w / 2.0, cy + h / 2.0),
    ...
    if !has_windows && rng.gen_range(0.0, 1.0) < 0.9 || has_windows && rng.gen_range(0.0, 1.0) < 0.2 {
    ...
    */
    ...
    routes.push(route.iter().map(|&(x, y)| {
...
fn rec<R: Rng>(rng: &mut R, depth: usize, base: &Vec<(f64, f64)>, maxy: f64) -> Vec<Vec<(f64, f64)>> {
    let mut routes = Vec::new();
    let first = base[0];
    ...
    let splits = if depth > 0 {
        (rng.gen_range(0., 0.08 * (last.0 - first.0)) * rng.gen_range(0.0, 1.0)) as usize
    } else { 0 };
    ...
    for i in 1..xs.len() {
        ...
        if w > rng.gen_range(3.0, 9.0) {
            ...
            let height = rng.gen_range(1.0, 20.0) + rng.gen_range(0., 60.) * rng.gen_range(0.0, 1.0);
            ...
            nextbase.push(lefth);
            let count = (rng.gen_range(0., 4. + 0.2 * w) * rng.gen_range(0., 1.)) as usize;
            for i in 0..count {
                ...
                y += rng.gen_range(0.0, 1.0) * rng.gen_range(0.0, 1.0) * rng.gen_range(0.0, 10.0) * f;
...
fn art(opts: Opts) -> Vec<Group> {
    ...
    for i in 0..3 {
        routes.push(ground.iter().map(|&(x, y)| {
