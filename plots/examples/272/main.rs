...
fn art(opts: Opts) -> Vec<Group> {
  ...
  let mut rng = rng_from_seed(71.738 * opts.seed);
  ...
  let f = |(x, y): (f64, f64)| {
    ...
    let res =
      (1. + rng.gen_range(0.0, 36.0) * rng.gen_range(0.0, 1.0)) as usize;
    let mut s = 100f64;
    let k = rng.gen_range(0.0, 0.4);
    for _i in 0..res {
      let mut p = (c.0, c.1);
      let ang = rng.gen_range(0f64, PI);
      let gap = rng.gen_range(0.02, 0.14);
      p.0 += rng.gen_range(-2.4 * gap, 2.4 * gap);
      p.1 += rng.gen_range(-gap, gap);
      p = p_r(p, ang);
      let dim = (rng.gen_range(0.05, 0.2), rng.gen_range(0.05, 0.2));
      s = op_union_round(s, sdf_box2(p, dim), k);
    }
    let f1 = rng.gen_range(0.1, 3.0);
    let f2 = rng.gen_range(1.0, 8.0);
    let f3 = rng.gen_range(2.0, 12.0);
    let a1 = rng.gen_range(0.0, 0.5)
      ...
      * perlin.get([
        ...
        opts.seed
          + a2
            * perlin.get([
              8. + opts.seed,
              f2 * c.0 + a3 * perlin.get([f3 * c.0, f3 * c.1, 10. + opts.seed]),
              f2 * c.1 + a3 * perlin.get([f3 * c.0, f3 * c.1, 20. + opts.seed]),
            ]),
      ]);
    lerp(-1.2, 0.15, s) + n
  };
