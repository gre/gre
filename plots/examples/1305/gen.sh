...
for page in $(seq 0 1 17); do
    cargo run --release --example 1304_wip -- --seed $seed --page $page --file results/svgs/seed_${seed}_${page}.svg &
done
wait
...
for page in $(seq 0 1 17); do
    convert -background black svgs/seed_${seed}_${page}.svg tmp/seed_${seed}_page_${page}.png &
done
wait
...
ffmpeg -y -r 30 -i tmp/seed_${seed}_%d.png -c:v libx264 -vf "pad=ceil(iw/2)*2:ceil(ih/2)*2,fps=8,format=yuv420p" videos/$seed.mp4
