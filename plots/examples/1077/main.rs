...
fn art(opts: &Opts) -> Vec<Group> {
  ...
  for c in circles {
    ...
    match c.shape {
      CircleShape::Spiral => {
        if c.r > 0.5 {
          routes.push(spiral_optimized(c.x, c.y, c.r, dr, 0.01));
        }
      }
      _ => {}
    };
...
fn rec_packing(
  ...
  let ppad =
    pad + rng.gen_range(0.0, 10.0) * rng.gen_range(-1.0, 1.0f64).max(0.0);
  let optimize =
    1 + ((rng.gen_range(0.0, 100.0) * rng.gen_range(0.0, 1.0)) as usize);

  let max_scale =
    (min_scale + 0.1f64).max(rng.gen_range(0.1, 0.9) * container.r);
