...
fn shapes(
    ...
    } in cycles
    {
        for _i in 0..count {
            let mut route = (0
                ..((resolution as f64 * stops) as usize))
                .map(|j| {
                    let p = j as f64 / (resolution as f64);
                    let a = a_off + p;
                    let ang = a * 2. * PI;
                    let amp =
                        amp_multiplier(a % 1.) * radius;
                    disp(
                        (
...
fn art(opts: Opts) -> Vec<Group> {
    ...
    let harmonies = vec![
        ...
        (
            ...
            rng.gen_range(0.5, 6.0f64).floor(),
        ),
    ];
    ...
    let conf = ShapeConfig {
        ...
        harmonies_mul: 1.,
        displacement: rng.gen_range(0., 0.1),
        disp_harmonies: disp_harmonies.clone(),
        seed: opts.seed,
    };

    let colors = vec!["green", "firebrick", "blueviolet"];
    let data_splitted =
        shapes(colors.len(), 0.331, 0.8, conf);
    ...
    colors
        ...
        .map(|(i, color)| {
            let data =
                data_splitted.get(i).unwrap().clone();
