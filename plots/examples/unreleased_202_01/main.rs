...
fn art(opts: Opts) -> Vec<Group> {
  ...
  let routes = packing(opts.seed, 1000000, 2000, 4, 0.0, &container, 0.6, 55.0)
    .par_iter()
    .map(|poly| {
      ...
      let mut candidates = sample_2d_candidates(
        ...
        400,
        20 + (2. * bounds.width() * bounds.height()).powf(0.8) as usize,
        &mut rng,
      );
  ...
  l = l.add(signature(0.8, (265.0, 200.0), color));
