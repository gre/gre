...
fn shape(path: &str, inside: bool, i: usize) -> Path {
  ...
  let threshold = 0.5;
  let f = |origin: (f64, f64)| grayscale(get_color(origin));
  let res = contour(width, height, f, &vec![0.4]);
  let mut routes = features_to_routes(res, precision);
  routes = crop_routes(&routes, bounds);
  let mut data = Data::new();
  let thresholds = if i == 5 {
    (0.02, 0.4, 0.5)
  } else {
    (0.08, 0.15, 0.4)
  };
  data = fill(data, bounds, (0.0, 0.8), f, thresholds.0, inside);
  data = fill(data, bounds, (0.8, 0.0), f, thresholds.0, inside);
  data = fill(data, bounds, (1.3, 0.0), f, thresholds.1, inside);
  data = fill(data, bounds, (0.0, 1.3), f, thresholds.1, inside);
...
fn art(_opts: &Opts) -> Vec<Group> {
  vec!["/Users/grenaudeau/Desktop/avatars/a.jpeg"]
    .iter()
    .enumerate()
    .map(|(i, path)| {
      let x = (i % 3) as f64 * 90. + 20.;
      let y = (i / 3) as f64 * 90. + 30.;
      Group::new()
        .set("transform", format!("translate({},{})", x, y))
        .add(shape(path, true, i))
    })
    .collect()
}
