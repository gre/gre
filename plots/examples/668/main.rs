...
fn art(opts: &Opts) -> Vec<Group> {
  ...
  let cpad = rng.gen_range(1.0, 2.0);
  let min_scale = cpad + rng.gen_range(1.0, 2.0);
  let max_scale = min_scale + rng.gen_range(0.0, 80f64).max(0f64);

  let xpadlayer = 22.0;
  let count = 2;

  let colors = vec!["#F90", "#F0B", "#0CF"];
  ...
  colors
    ...
    .map(|(i, color)| {
      ...
      for j in 0..count {
        let index = i * count + j;
        let xpad = index as f64 * xpadlayer;
        let bound = (pad + xpad, pad, width - pad - xpad, height - pad);
        let max_scale_f = |(x, y): (f64, f64)| {
          mix(
            min_scale, max_scale,
            1.0, //(2.0 * (((y / height) - 0.5).abs())).powf(6.0),
          )
        };
        let circles = packing(
          opts.seed * 7.7 + index as f64 / 3.0,
          1000000,
          5000,
          1,
          cpad,
        ...
        for c in circles {
          ...
          loop {
            ...
            let res = r as usize * 2 + 10;
            routes.push(circle_route((c.x, c.y), r, res));
            routes.push(circle_route((c.x, c.y), r - 0.4, res));
            r -= 2.0;
          }
          // routes.push(spiral(c.x, c.y, c.r, 2.0));
        }
      }
