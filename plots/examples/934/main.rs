...
pub struct Opts {
  ...
  pub precision: f64,
  #[clap(short, long, default_value = "2.03")]
  pub size: f64,
  #[clap(short, long, default_value = "2.2")]
  pub word_dist: f64,
  #[clap(short, long, default_value = "3.4")]
  pub radius: f64,
...
fn main() {
  ...
  // If this was never done yet, this will pull from twitter api all followers
  ...
  // we extract all twitter usernames from the file, and make art with it
...
fn art(
  ...
  let get_value = |(x, y): (f64, f64)| {
    let px = x / width;
    let py = (y - (height - width) / 2.0) / width;
