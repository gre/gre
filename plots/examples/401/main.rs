...
fn main() {
    ...
    let dash_length = rng.gen_range(20.0, 100.0);
    let amp: f64 = rng.gen_range(0.4, 1.);
    let color_alt = ((rng.gen_range(0.0, 1.0) * rng.gen_range(0.0, 200.0)) as usize) + 1;
    let angle_velocity_field = |x, y, l, _length| {
        (0.5-(l as f64 - 0.5).abs()) * amp * (
            0.5 * perlin.get([seed, 7.0 * x, 7.0 * y])
            + 0.3 * perlin.get([11.0 * x, seed, 11.0 * y])
            + 0.2 * perlin.get([21.0 * x, 21.0 * y, seed]))
    };
    let w = rng.gen_range(6., 12.);
    let wf = rng.gen_range(0.5, 2.0);
    let origin = |l| {
        (
