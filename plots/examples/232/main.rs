...
fn contouring_circle(seed: f64, circle: &VCircle) -> Vec<Vec<(f64, f64)>> {
  let precision = 0.2;
            ...
            0.4 * seed
              + 0.3
                * perlin.get([
                  6. * x,
                  6. * y,
                  10. + seed + 0.2 * perlin.get([30. * x, 30. * y, seed]),
                ]),
          ]),
        0.0,
  ...
  routes = routes
    ...
    .map(|route| {
      route
        .iter()
        .map(|p| (circle.x - circle.r + p.0, circle.y - circle.r + p.1))
        .collect()
    })
    .collect();
  routes
...
fn art(opts: Opts) -> Vec<Group> {
  ...
  let routes = primaries
    ...
    .map(|circle| {
      contouring_circle(opts.seed + circle.x * 9. + circle.y / 29., circle)
    })
