...
fn parametric(p: f64, opts: &Opts) -> (f64, f64) {
  ...
  (
    a * (2.0 * PI * p).cos() + (1.0 - a) * (f1 * PI * p).cos(),
    a * (2.0 * PI * p).sin() + (1.0 - a) * (f2 * PI * p).sin(),
  )
}

fn art(opts: &Opts) -> Vec<Group> {
  let colors = vec![
    "red", "cyan", "darkblue", "#F90", "black", "pink", "green", "purple",
  ];
  ...
  let get_angle = |p, initial_angle, length| -> f64 {
    initial_angle + 0.5
      - length * 0.00005 * euclidian_dist(p, (width / 2., height / 2.))
  };
  ...
  let mut build_route = |p: (f64, f64), l, route_i| {
    ...
    let angle = get_angle(p, initial_angle, l as f64 * granularity);
    let next = (
