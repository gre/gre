...
fn art(opts: &Opts) -> Vec<Group> {
  ...
  let gridw = rng.gen_range(4, 16);
  let gridh = rng.gen_range(3, 12);
  let secondary_threshold = 6.0;
  let min_threshold = 4.;
  ...
  for &c in primaries.iter() {
    let min_c = min_threshold + rng.gen_range(0.0, 2.0 * secondary_threshold);
    let max_c = 1.4 * min_c + 8.0;
    ...
    let routes: Vec<Vec<(f64, f64)>> = samples
      ...
      .map(|(si, &sample)| {
        ...
        loop {
          ...
          ang += 0.5 + perlin.get([xi as f64 * 0.2, yi as f64 * 0.2, seed]);
