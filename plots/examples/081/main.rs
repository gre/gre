  ...
  colors
    ...
    .map(|(i, color)| {
      ...
      let field = |(x, y): (f64, f64), l: f64| {
        (if i == 0 { 3.3 } else { 3.0 })
          * perlin.get([2.0 * x, 2.0 * y, opts.seed])
          + 1.2 * perlin.get([5.0 * x, 5.0 * y, 1.0 + opts.seed])
          + 1.0 * perlin.get([16.0 * x, 16.0 * y, 2.0 + opts.seed + l * 0.05])
      };
          ...
          (
            ...
            boundaries.1
              + (boundaries.3 - boundaries.1)
                * (0.5
                  + 0.3 * (PI * p).sin() * (i as f64 * 0.5 + l as f64).cos()),
          )
        })
        .collect();
      ...
      let mut build_route = |p: (f64, f64), l, route_i| {
        ...
        let ends =
          passage > 4 || i > length || out_of_boundaries(next, boundaries);
        if ends {
          None
        } else {
          Some((next, false))
