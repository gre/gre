...
fn art(opts: &Opts) -> Vec<Group> {
    ...
    .map(|(g, &clr)| {
        ...
        for i in 0..samples {
            ...
            let rgb = get_color(preserve_ratio_outside(
                pn,
                (width, height),
            ));
            let dist = euclidian_rgb_distance(clr, rgb)
                + if g == 3 { 0.5 } else { 0.0 };
            let draw = dist
                < 1.4
        ...
        let data = routes
            .iter()
            .fold(Data::new(), |acc, route| {
                render_route(acc, route.clone())
            });

        let mut l = layer(name).add(
            base_path(name, 0.2, data).set("opacity", 0.8),
        );
        if g == 2 {
            l = l.add(signature(
                1.0,
...
struct Opts {
    #[clap(
        short,
        long,
        default_value = "images/pattern_02_first_mint.png"
    )]
    path: String,
