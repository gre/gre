...
fn art(opts: Opts) -> Vec<Group> {
  ...
  let len = palette.len();
  let colors =
    vec![&palette[opts.index % len], &palette[(opts.index + 1) % len]];
  let p = opts.index as f64 / opts.frames as f64;
  let p1 = (p * PI * 2.).sin();
  let p2 = (p * PI * 2.).cos();

  let mut passage = Passage2DCounter::new(passage_precision, 297., 210.);

  let perlin = Perlin::new();
  let field = |(x, y): (f64, f64), l: f64| {
    // "domain warp" technique over perlin
    let (qx, qy) = (
      perlin.get([0.5 * x, 0.5 * y, 10. + seed + opts.delta1 * p2]),
      perlin.get([x + 2.7, y + 1.3, 20. + seed]),
    );
    let (rx, ry) = (
      perlin.get([x + 2. * qx + 0.13, y + 2. * qy + 0.31, 30. + seed]),
      perlin.get([x + 2. * qx + 0.347, y + 2. * qy + 0.858, 40. + seed]),
    );
    opts.amp
      * perlin.get([
        opts.freq * x + opts.rfreq * rx,
        opts.freq * y + opts.rfreq * ry + opts.delta2 * (x + p1),
        7.6 * seed + l,
      ])
  };
  ...
  let initial_positions = (0..lines)
    .map(|l| {
      ...
      (
        boundaries.0 + (boundaries.2 - boundaries.0) * p,
        boundaries.1
          + (boundaries.3 - boundaries.1)
            * (0.5 + 0.3 * (PI * p).sin() * (l as f64).cos()),
      )
    })
    .collect();
  ...
  let mut build_route = |p: (f64, f64), l, route_i| {
    ...
    let b = collide_segment_boundaries(p, next, boundaries);
    if let Some(c) = b {
      return Some((c, true));
    }
