...
fn art(opts: &Opts) -> Vec<Group> {
  ...
  let max_scale = rng.gen_range(0.1, 0.6) * height;
        ...
        let a = 2.0
          * perlin.get([
            ...
            opts.seed
              + perlin.get([
                2.0 * freq * p.0,
                1.0 * freq * p.1 + 0.1 * (0.1 * p.1).round(),
                opts.seed,
              ]),
          ]);
