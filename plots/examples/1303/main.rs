...
pub struct Opts {
  ...
  pub pad: f64,
  #[clap(short, long, default_value = "514.0")]
  pub seed: f64,
}
...
fn art(opts: &Opts) -> Vec<Group> {
  ...
  let min = rng.gen_range(20.0, 40.0);
  let max = min * rng.gen_range(1.1, 2.0);
  ...
  let count = rng.gen_range(30, 150);
  ...
  let count = rng.gen_range(30, 150);
  let min = rng.gen_range(5.0, 10.0);
  let max = min * rng.gen_range(1.1, 2.0);

  let circles3 = packing(
    &mut rng,
    500000,
    count,
    1,
  ...
  let clouds3: Vec<VCircle> = circles3
    .iter()
    .flat_map(|c| {
      let (rts, circles) = cloud_in_circle(&mut rng, &c);
      let rts = clip_routes(
...
fn cloud_in_circle<R: Rng>(
  ...
  let count = (circle.r * rng.gen_range(0.8, 1.2)).min(120.0) as usize
    + rng.gen_range(15, 30);
  for _i in 0..count {
