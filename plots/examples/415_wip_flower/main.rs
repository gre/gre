...
impl VCircle {
  ...
  fn dist_p(self: &Self, p: (f64, f64)) -> f64 {
      ...
      for j in 0..iterations {
        let mut d = amp
          * perlin.get([
            ...
            0.1 * perlin.get([0.002 * x, 0.002 * y, seed]),
          ]);
        acc_diff += d;
        if j > iterations / 2 {
          if (acc_diff < 0.) == (d < 0.) {
...
fn art(opts: Opts) -> Vec<Group> {
  ...
  for c in primary {
    if c.r > 2. * min_scale {
      for c2 in packing(
        seed, 2000000, 10000, 1, 0., bounds, &c, min_scale, max_scale,
      ) {
        circles.push(c2);
      }
      seed = seed * 1.1 + 0.3;
  ...
  let routes: Vec<Vec<(f64, f64)>> = circles
    ...
    .enumerate()
    .flat_map(|(i, c)| {
      flower(opts.seed * 7.7 + i as f64 / 3.1, (c.x, c.y), c.r)
    })
    .collect();
  ...
  layers.push(layer(color).add(base_path(color, stroke_width, data)));
  ...
  let f = |p: (f64, f64)| {
    ...
    let mut min: f64 = 99999.;
    for c in circles.iter() {
      min = min.min(c.dist_p(g) + c.r * 0.5);
    }
    let n = perlin.get([seed, p.0 * 30., p.1 * 30.]);
    min / 100.0 + 0.05 * n
  };
  let count = 30;
  let thresholds = (0..count).map(|i| i as f64 / (count as f64)).collect();
  let features = contour(w, h, f, &thresholds);
  // let routes = features_to_routes(features, precision);
  let routes = crop_routes(&features_to_routes(features, precision), bounds);
