...
fn art(opts: &Opts) -> svg::Document {
  ...
  let paper = grey_paper;
  let inks = vec![black, white_gel];
  ...
  for (ink_i, _ink) in inks.iter().enumerate() {
    let density = if ink_i == 0 { 12.0 } else { 5.0 };
    ...
    map.fill_fn(|(x, y)| {
      ...
      let c = img(p);
      let value = if ink_i == 0 {
        (1.0 - grayscale(c)).powf(1.4) - 0.1
      } else {
        grayscale(c).powf(1.4) - 0.3
      };
