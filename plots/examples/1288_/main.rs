...
fn art(opts: &Opts) -> Vec<Group> {
  ...
  let f1 = rng.gen_range(0.6, 6.0);
  let f2 = rng.gen_range(0.6, 6.0);
  let f3 = rng.gen_range(0.6, 6.0);
  ...
  for i in 0..2 {
    ...
    let valuef = Value2D {
      ...
      seed: opts.seed,
      count,
      symx: rng.gen_bool(0.3),
      symy: rng.gen_bool(0.3),
      rotation,
    };
    values.push(valuef.clone());

    let f = |p: (f64, f64)| valuef.map(p.0, p.1);
    ...
    for l in features_to_routes(contour(w, h, &f, &thresholds), precision) {
      routes.push((0, l));
    }
  }
