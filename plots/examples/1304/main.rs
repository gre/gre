...
pub struct Opts {
  ...
  pub precision: f64,
  #[clap(short, long, default_value = "2.1")]
  pub size: f64,
  ...
  #[clap(
    short,
    long,
    default_value = "/Users/gre/Documents/insta-fetch/list.txt"
  )]
  list_file: String,
  #[clap(short, long, default_value = "images/letters.svg")]
  letters_file: String,
  #[clap(short, long, default_value = "images/insta1k.png")]
  image_layer: String,
  #[clap(short, long)]
  debug: bool,
}
...
fn main() {
  ...
  let file_content = std::fs::read_to_string(list_file.clone()).unwrap();

  // we will then load our custom "font" from the SVG reference that contains all plottable letters (made by @greweb)
  ...
  // we extract all usernames from the file, and make art with it
...
fn art(
  ...
  data
    ...
    .map(|(name, routes)| {
      ...
      l = l.add(base_path("black", 0.35, data));
      l
    })
    .collect()
}
