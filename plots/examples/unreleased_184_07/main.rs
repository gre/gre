...
fn waves_in_circle(
  ...
  let offset_y = 10.0 + 0.5 * circle.r;
            ...
            seed
              + 0.8
                * perlin.get([
                  0.02 * x
                    + 0.1
                      * perlin.get([0.1 * x, 0.1 * base_y, 100. + seed / 3.0]),
                  0.08 * base_y,
                  10. + seed / 7.0,
                ]),
          ]);
...
fn waveball(n: usize, seed: f64, c: &VCircle) -> WaveballRes {
  ...
  let (waves, height_map) = waves_in_circle(seed, c, 0.2, 0.8);

  let res = packing(seed, 100000, 500, 4.0, c, &height_map, 4.0, 100.0)
    .par_iter()
...
fn art(opts: Opts) -> Vec<Group> {
  ...
  l = l.add(signature(0.8, (185.0, 225.0), color));
  layers.push(l);
