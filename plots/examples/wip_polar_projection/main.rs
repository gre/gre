  ...
  fn draw_increment(&mut self, value: &Value, index: usize) -> ArtIncrement {
    ...
    if self.sky_should_enable_next_time {
      let bound = (
        ...
        self.args.height - self.args.padding,
      );
    ...
    self.sky_should_enable_next_time =
      self.sky.is_none() && self.mountains.has_finished();
...
impl Mountains {
  ...
  fn iterate(
    ...
    let rbase = self.rbase;

    let mut out_of_bounds = false;

    let mut route = curve_values
      .iter()
      .enumerate()
      .map(|(i, &v)| {
        ...
        let p = (x, y);
        if gre::out_of_boundaries(p, bound) {
          out_of_bounds = true;
        }
        p
      })
    ...
    let is_outside = |p: (f64, f64)| collision.collides(p);
    routes.push(route.clone());
    routes = clip_routes(&routes, &is_outside, 1.0, 3);

    for tuple in route.windows(2) {
      collision.add_polygon(vec![tuple[0], tuple[1], (cx, cy)]);
    }
...
impl ShapeCollision {
  ...
  fn add_polygon(&mut self, polygon: Vec<(f64, f64)>) {
    self.polygons.push(polygon);
  }

  fn add_circle(&mut self, circle: VCircle) {
    self.circles.push(circle);
  }

  fn collides(&self, p: (f64, f64)) -> bool {
...
impl ClippedShapeBuilder {
  ...
  fn is_outside(&self, p: (f64, f64)) -> bool {
    ...
    p.0 < x1 || p.0 > x2 || p.1 < y1 || p.1 > y2 || self.collision.collides(p)
  }
  ...
  fn iterate(
    ...
    if uses_circle {
      self
        .collision
        .add_circle(VCircle::new(self.pos.0, self.pos.1, self.scale));
    } else {
      self.collision.add_polygon(route);
    }
