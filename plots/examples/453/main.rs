...
fn art(opts: &Opts) -> Vec<Group> {
    ...
    let min_route = 8;
    let res1 =
        (rng.gen_range(0.5, 4.0), rng.gen_range(1.0, 3.0));
    let res2 =
        (rng.gen_range(0.5, 4.0), rng.gen_range(1.0, 3.0));
    ...
    let low_poly_perlin =
        |(xr, yr): (f64, f64), x: f64, y: f64, s: f64| {
    ...
    for (ci, &color) in colors.iter().enumerate() {
        ...
        loop {
            ...
            loop {
                ...
                let xv =
                    (base_y / height) * (x - width / 2.);
                let amp = mix(0.8, 2.0, r1)
                    * height
                    * (base_y / height);
