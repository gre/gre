    ...
    colors.iter().enumerate().map(|(ci, &color)| {
        ...
        for r in vec![-1.0, 1.0] {
            let f = |(x, y): (f64, f64)| {
                ...
                let n = 0.05 * rng.gen_range(0.0, 1f64) * perlin.get([
                    f1 * c.0,
                    f1 * c.1,
                    100.0 + opts.seed * 7.3
                    + a2 * perlin.get([
            ...
            let inside = |from: (f64, f64), to| {
                ...
                perlin.get([
                    ...
                    + rng.gen_range(0.0, 1.0) * perlin.get([
                        0.7 + opts.seed * 6.3 + rng.gen_range(0.0, 1.0) * perlin.get([opts.seed, nz3 * to.0, nz3 * to.1]),
                        nz2 * from.0,
                        nz3 * from.1,
                    ])
