...
fn packing_circles(
    ...
    for _i in 0..iterations {
        ...
        if let Some(size) = search_circle_radius(&container, &all, Vec::new(), x, y, min_scale, max_scale) {
...
fn scaling_search_in_container<F: FnMut(f64, f64, f64, f64) -> Polygon<f64>>(
    ...
    let overlaps = |size| {
        ...
        poly.exterior().points_iter().all(|p| container.includes(p.x_y())) &&
...
fn squares(
    ...
    let polygons = poly_accumulate(p, &rotated_square_as_polygon, seed, 200000, 5000, 0.4, circle, 0.6);
    let routes = polygons.iter().map(|poly| {
        poly.exterior().points_iter().map(|p| p.x_y()).collect()
    }).collect();
...
fn art(opts: Opts) -> Vec<Group> {
    ...
    let primaries = packing_circles(vec![c1, c2, c3], opts.seed, 100000, 1000, 1.0, &mainc, 1.0, 50.0);
