...
fn art(opts: &Opts) -> Vec<Group> {
  ...
  let rows = rng.gen_range(22, 28);
  let divs = rng.gen_range(25, 40);

  let w = (width - 2. * pad) / (divs as f64 - 1.0);
  let h = (height - 2. * pad) / (rows as f64);
  ...
  for r in 0..rows {
    let yc = pad + h * (r as f64 + 0.5);
    let mut line1 = vec![];
    let mut line2 = vec![];
    for i in 0..divs {
      let edge = i == 0 || i == divs - 1;
      let x = pad + w * (i as f64);
      let x1 = if edge {
        x
      } else {
        x + dispx * rng.gen_range(-1.0, 1.0) * w
      };
      let x2 = if edge {
        x
      } else {
          ...
          * if edge {
            ...
            rng.gen_range(0.0, 1.0)
              + spikeamp
                ...
                * rng.gen_range(0.0f64, 1.0).powi(spikepow)
          };
      let y2 = yc
        + h
          * dispy
    ...
    line2.reverse();
    let poly = vec![line1, line2].concat();
    polygons.push(poly);
  }
