...
fn art(opts: &Opts) -> Vec<Group> {
  ...
  let cpad = rng.gen_range(1.0, 5.0);
  let min_scale = cpad + rng.gen_range(1.0, 10.0);
  let max_scale = min_scale + rng.gen_range(-20.0, 100f64).max(0f64);
  ...
  let routes = (0..count)
    ...
    .map(|i| {
      ...
      let dr = 0.4 + (count - 1 - i) as f64 / 4.0;
      ...
      for c in circles {
        routes.push(circle_route((c.x, c.y), c.r, 400));
        routes.push(spiral(c.x, c.y, c.r, dr));
      }
      routes
    })
    .collect::<Vec<_>>()
