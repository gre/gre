...
fn art(opts: Opts) -> Vec<Group> {
  ...
  let configs = vec![(
    ...
    ShapeConfig {
      ...
      cycles: vec![
        Cycle {
          count: rng.gen_range(40, 100),
          vradius: rng.gen_range(-0.5, -0.3),
          vx: rng.gen_range(-0.5, 0.5),
          vy: rng.gen_range(-0.5, 0.5),
        },
        Cycle {
          count: rng.gen_range(20, 60),
          vradius: rng.gen_range(-1.0, -0.6),
          vx: rng.gen_range(-0.5, 0.5),
          vy: rng.gen_range(-0.5, 0.5),
        },
      ],
      harmonies: vec![
        (
          ...
          rng.gen_range(8.0, 20.0f64).floor(),
        ),
        (
          rng.gen_range(0.02, 0.06),
          rng.gen_range(1.0, 6.0f64).floor(),
        ),
      ],
      displacement: rng.gen_range(0.0, 0.02),
      disp_harmonies: vec![
