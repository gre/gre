...
fn art(opts: Opts) -> Vec<Group> {
  ...
  let get_color = image_get_color("images/coffeecup.jpeg").unwrap();
  ...
  colors
    ...
    .map(|(ci, color)| {
      ...
      let f = |p| {
        ...
        c.powf(1.6) * smoothstep(0.6, 0.2, dist)
      };
      ...
      for p in samples {
        ...
        let b =
          follow_angle(a, 0.5 * PI + (p.1 - 0.4).atan2(p.0 - 0.5), stroke_dist);
        data = data.move_to(a).line_to(b);
      }
      l = l.add(base_path(color, 0.35, data));
      if ci == colors.len() - 1 {
        l = l.add(signature(0.8, (210.0, 188.0), color));
      }
      l
    })
    .collect()
