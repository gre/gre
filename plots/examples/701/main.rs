...
pub struct Opts {
  ...
  pub pad: f64,
  #[clap(short, long, default_value = "1527.0")]
  pub seed: f64,
...
fn art(opts: &Opts) -> Vec<Group> {
  ...
  let peakfactor = rng.gen_range(0.001, 0.01);
  let stopy = rng.gen_range(0.6, 0.9) * height;
  let ampfactor = rng.gen_range(0.01, 0.02);
  let ynoisefactor = rng.gen_range(0.05, 0.2);
  let yincr = rng.gen_range(0.3, 0.4);
  let amp2 = rng.gen_range(6.0, 10.0);
  let ymul = rng.gen_range(0.001, 0.002);
      ...
      if inside && passage.get((x, y)) < passage_threshold {
        if was_outside {
          if route.len() > min_route {
            routes.push((Color::Mountain, route));
          }
          route = Vec::new();
        }
        was_outside = false;
  ...
  let balanced = rng.gen_bool(0.3);
  let proba_a = if balanced {
    0.33
  } else {
    rng.gen_range(-0.2, 1.2f64).max(0.0001).min(0.9999)
  };
  let proba_b = if balanced {
    0.5
  } else {
    rng.gen_range(-0.2, 1.2f64).max(0.0001).min(0.9999)
  };
  ...
  let radius = (0.1 + (1.0 - m) * rng.gen_range(0.2, 0.4)) * height;
  let center = (
    width / 2.0,
    stopy - radius - rng.gen_range(0.0, 0.2) * height,
  );
  let mut flowers = Vec::new();
  let count = (1.0 + rng.gen_range(0.0, 100.0 * m)) as usize;
  let min_size = rng.gen_range(0.08, 0.2);
  let max_size = rng.gen_range(0.0, 0.3 + 0.5 * m);
  for _i in 0..count {
    let x =
      center.0 + rng.gen_range(-1.0, 1.0) * rng.gen_range(0.2, 1.0) * radius;
    let r = radius
      * (min_size + rng.gen_range(0.0, max_size) * rng.gen_range(0.0, 1.0));
    let y = (center.1
      + rng.gen_range(-1.0, 1.0) * rng.gen_range(0.2, 1.0) * radius)
      .min(height_map[(x / precision) as usize % height_map.len()] - r - 5.0);
    flowers.push(VCircle::new(x, y, r));
    let count =
      (density * rng.gen_range(0.9, 1.1) * r.powf(1.4) + 10.0) as usize;
    ...
    let clr = if rng.gen_bool(proba_a) {
      Color::Flower1
    } else if rng.gen_bool(proba_b) {
      Color::Flower2
    } else {
      Color::Flower3
    };
    ...
    for &q in points.iter().skip(1) {
      ...
      loop {
        ...
        let p = lerp_point(p, q, v / l);
        if passage.count(p) < 7
          && strictly_in_boundaries(p, (pad, pad, width - pad, height - pad))
        {
          route.push(p);
        } else {
          if route.len() > 1 {
  ...
  let diff = rng.gen_range(0.0, 0.05) * rng.gen_range(0.0, 1.0) * radius;
  let shake_factor = rng.gen_range(0.0, 2.0);
  for flower in flowers {
    let xc =
      width * 0.5 + rng.gen_range(-1.0, 1.0) * diff * rng.gen_range(0.5, 1.0);
    let target = (xc, height_map[(xc / precision) as usize % height_map.len()]);
    let mid = (target.0, mix(flower.y, target.1, rng.gen_range(0.4, 0.6)));

    let points = path_subdivide_to_curve(
      shake(
        path_subdivide_to_curve(
          vec![(flower.x, flower.y), mid, target],
          2,
          0.75,
        ),
        rng.gen_range(0.8, 1.2) * shake_factor,
        &mut rng,
      ),
      1,
      0.8,
    ...
    for &q in points.iter().skip(1) {
      ...
      loop {
        ...
        let p = lerp_point(p, q, v / l);
        if passage.count(p) < 3
          && strictly_in_boundaries(p, (pad, pad, width - pad, height - pad))
        {
          route.push(p);
        } else {
          if route.len() > 1 {
            routes.push((Color::Grass, route));
          }
          route = Vec::new();
        }
        v += incr;
  ...
  let total_pad = pad + radius + border_dist * (border_size as f64);
  let ppad = 1.0;
  let min = ppad + rng.gen_range(0.3, 0.5);
  let max = min + rng.gen_range(0.0, 3.0);
  let circles = packing(
    opts.seed,
    1000000,
    2000,
  ...
  for c in circles {
    ...
    routes.push((Color::Mountain, circle_route((c.x, c.y), c.r, count)));
  }
