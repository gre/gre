...
fn art(opts: Opts) -> Vec<Group> {
    ...
    for (color, from, to, samples) in themes {
        ...
        let f = |(x, y): (f64, f64)| {
            let mut d = 1000f64;
            let mut g = project_in_boundaries((x, y), (pad, pad, width-pad, height-pad));
            let a = 2. * PI * perlin.get([
                x,
                y,
                0.4 * seed + 0.5 * perlin.get([
                    2. * x,
                    2. * y,
                    10. + seed + 0.5 * perlin.get([
                        10. * x,
                        10. * y,
                        seed
                    ])
            ...
            d += 2.0 * perlin.get([
                2. * x,
                2. * y,
                7.4 * seed + 0.5 * perlin.get([
                    4. * x,
                    4. * y,
                    7. + 8.8 * seed + 0.5 * perlin.get([
                        7. * x,
                        7. * y,
                        7.7 * seed
                    ])
