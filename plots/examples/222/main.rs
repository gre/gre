...
fn shape(seed: f64, circle: &VCircle) -> (Vec<Vec<(f64, f64)>>, Vec<f64>) {
  ...
  routes = routes
    ...
    .map(|r| {
      r.iter()
        .map(|p| (p.0 + boundaries.0, p.1 + boundaries.1))
        .collect()
    })
    .collect();
  ...
  loop {
    ...
    loop {
      ...
      let mut y = base_y;
      y += (circle.r - euclidian_dist((circle.x, circle.y), (x, base_y)))
        .max(0.)
        * 0.15
        * perlin.get([
          0.1 * x,
            ...
            + 2.
              * perlin.get([
                0.1 * y
                  + 0.8
                    * perlin.get([
                      seed + 1.6 * perlin.get([0.5 * y, 0.1 * x, seed / 7.]),
                      0.8 * y,
                      0.8 * x,
                    ]),
                0.03 * x,
