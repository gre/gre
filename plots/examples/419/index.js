...
function art(S) {
  ...
  let curves = [];
  let pad = 40;
  let origin = [(WIDTH - sentenceWidth * scale) / 2, scale * 140];
  let xamp = WIDTH / 10;
  let ymult = 0.12;
  let y = origin[1];
  ...
  const newline = () => {
    let dc =
      2 *
      Math.min(1, Math.max(0, Math.abs((1.2 * (y - HEIGHT * 0.4)) / HEIGHT)));
    y += 1 + 50 * Math.pow(dc, 6);
  };
  while (!endofpage) {
    x = origin[0] + xamp * Math.cos(y * ymult);
    let shaking = Math.max(0, 40 * (1 - (1.2 * y) / HEIGHT));
    for (let i = 0; i < sentence.length; i++) {
      let letter = characters[sentence[i]] || SPACE;
      let c = letter.curve;
