...
fn art(opts: &Opts) -> Vec<Group> {
  ...
  let gridw = rng.gen_range(4, 12);
  let gridh = rng.gen_range(3, 9);
  let min_threshold = 8.0;
  ...
  let mut circles = packing(
    ...
    min_threshold,
    rng.gen_range(min_threshold, 6. * min_threshold),
  );
    ...
    let routes: Vec<Vec<(f64, f64)>> = samples
      ...
      .map(|(si, &sample)| {
        ...
        loop {
          ...
          ang += 0.5 + perlin.get([xi as f64 * 0.1, yi as f64 * 0.1, seed]);
              ...
              2.4 * seed
                + rng.gen_range(0.0, 3.0)
                  * perlin.get([
                    f2 * 0.04 * g.0,
                    f2 * 0.04 * g.1,
                    seed + 0.2 * perlin.get([f3 * 0.08 * g.0, f3 * 0.08 * g.1, seed]),
                  ]),
            ]);
