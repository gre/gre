...
fn art(opts: &Opts) -> Vec<Group> {
  ...
  for _i in 0..iterations {
    ...
    let scale = rng.gen_range(2.5, 8.0);
    let rot = PI * perlin.get([opts.seed, 0.01 * x as f64, 0.01 * y as f64]);
    let color = rng.gen_range(0, inks.len());
    let rts = crab((x, y), scale, rot, color, &mut rng);
    let mut local_passage = Passage::new(precision, width, height);
    let mut out_of_bounds = false;
    for (_ci, route) in rts.clone() {
      if out_of_bounds {
        break;
      }
      for r in route.clone() {
        if !strictly_in_boundaries(r, bound) || passage.get(r) > 0 {
          out_of_bounds = true;
          break;
        }
  ...
  inks
    ...
    .map(|(ci, ink)| {
      ...
      let mut l = layer(ink);
      l = l.add(base_path(ink, 0.35, data));
      l
    })
    .collect()
}
