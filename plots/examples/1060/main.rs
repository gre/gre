...
pub struct Opts {
  ...
  pub pad: f64,
  #[clap(short, long, default_value = "607.0")]
  pub seed: f64,
}
...
fn art(opts: &Opts) -> svg::Document {
  ...
  let white = Ink("White", "#ccc", "#fff", 0.35);
  let black_paper = Paper("Black", "#222", false);

  let paper = black_paper;
  let inks = vec![white];
  ...
  let f2 = f1 * rng.gen_range(0.0, 2.0);
  let amp2 = rng.gen_range(1.0, 4.0);
  let f3 = f2 * rng.gen_range(0.0, 2.0);
  let amp3 = rng.gen_range(1.0, 4.0) * rng.gen_range(0.0, 1.0);
  let f4 = rng.gen_range(0.5, 4.0);
  let amp4 = rng.gen_range(-0.2f64, 0.8).max(0.0);
  ...
  let f = |(x, y): (f64, f64)| -> (usize, f64) {
    ...
    let rotated = p_r((x, y), PI / 4.0);

    let xi = (1000.0 + rotated.0 / r) as i32;
    let yi = (1000.0 + rotated.1 / r) as i32;

    // let valuemod = (((x / r) % 2.0).floor() + ((y / r) % 2.0).floor()) % 2.0;
    ...
    let value1 = if n1 < 0.0 { -0.1 * n1 } else { 0.3 + 0.7 * n1 };
    let value = modfactor * value1;
    let color = 0;
    (color, value * density)
  };
