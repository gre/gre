...
fn search(
    ...
    let overlaps = |size| {
        ...
        p.exterior().points_iter().all(|c| container.contains(&c))
...
fn rec(
    ...
    let mut rng = rng_from_seed(seed);
    let nextpolygoncount = if rng.gen_bool(0.8) {
        polygoncount
    } else {
        rng.gen_range(3, 6)
    };
    let min = 1.0 + 0.5 * (((2 + depth.min(1)) % 3) as f64) + 0.2 * (depth as f64);
    let pad = 0.5 * min;
    let mut routes: Vec<Vec<(f64, f64)>> = packing(
        ...
        400000,
        3000,
        1 + (rng.gen_range(depth as f64 * 80.0, 20.0 + depth as f64 * 160.0)
    ...
    .par_iter()
    .flat_map(|poly| rec(depth + 1, nextpolygoncount, 7.6 + seed * 3.3, &poly))
    .collect();
    routes.push(
        container
            .exterior()
...
fn art(opts: &Opts) -> Vec<Group> {
    ...
    let routes = rec(0, polygoncount, opts.seed, &container);
