...
fn search(
  ...
  let overlaps = |size| {
    ...
    container.contains(p) && !poly_collides_in_polys(polys, p)
  };
...
fn art(opts: Opts) -> Vec<Group> {
  ...
  let routes = packing(opts.seed, 100000, 2000, 8, 0.1, &container, 0.6, 100.0)
    .par_iter()
    .map(|poly| {
      ...
      let mut candidates = sample_2d_candidates_f64(
        &|p| {
          let q = f(p);
          if poly.intersects(&Point::from(q)) {
            return euclidian_dist(p, (0.5, 0.0));
          } else {
            return 0.0;
          }
        },
        400,
        10 + (0.3 * bounds.width() * bounds.height()).powf(0.66) as usize,
        &mut rng,
      );
  ...
  l = l.add(signature(0.8, (266.0, 199.0), color));
