    ...
    for _i in 0..2 {
        routes.push(
            route
                .iter()
                .map(|&(x, y)| {
                    let dx = rng.gen_range(-1.0, 1.0) * 0.6;
                    let dy = rng.gen_range(-1.0, 1.0) * 0.6;
                    (x + dx, y + dy)
                })
                .collect(),
        );
...
fn rec<R: Rng>(
    ...
    let splits = if depth > 0 {
        (rng.gen_range(0., 0.5 * (last.0 - first.0)) * rng.gen_range(0.0, 1.0)) as usize
    } else {
        0
    };
    ...
    for i in 1..xs.len() {
        ...
        if w > rng.gen_range(3.0, 9.0) {
            ...
            let height = rng.gen_range(1.0, 8.0) + rng.gen_range(0., 40.) * rng.gen_range(0.0, 1.0);
...
fn art(opts: &Opts) -> Vec<Group> {
    ...
    let shapes = rec(&mut rng, 12, &ground, pad);
