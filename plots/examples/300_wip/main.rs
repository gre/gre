...
fn art(opts: Opts) -> Vec<Group> {
    ...
    let colors = vec!["magenta", "cyan", "yellow"];
    colors.iter().enumerate().map(|(ci, &color)| {
        let f = |(x, y): (f64, f64)| {
            ...
            let res = rng.gen_range(2, 200);
            let mut s = 100f64;
            let k = rng.gen_range(0.0, 0.2);
            for _i in 0..res {
              let mut p = (c.0, c.1);
              let ang = rng.gen_range(0f64, PI);
              p.1 += rng.gen_range(-0.5, 0.5) * rng.gen_range(0.0, 1.0);
              p.0 += rng.gen_range(-0.2, 0.2) * rng.gen_range(0.0, 1.0);
              p = p_r(p, ang);
              let dim = (
            ...
            let n = rng.gen_range(0.0, 1.0) * perlin.get([
                f1 * c.0,
                f1 * c.1,
                opts.seed
                - rng.gen_range(0.0, 2.0f64) * rng.gen_range(0.0, 1.0f64).powf(2.0) * (ci as f64)
                - a2 * perlin.get([
                    opts.seed
                    + rng.gen_range(0.0, 2.0f64) * rng.gen_range(0.0, 1.0f64).powf(2.0) * (ci as f64),
