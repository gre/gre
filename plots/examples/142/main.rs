  ...
  colors
    ...
    .map(|(ci, color)| {
      ...
      let mut pts = (0..opts.points)
        .map(|i| {
          let r = rng.gen_range(opts.radius_from, opts.radius_to);
          let a =
            PI / 2. + PI * (2. * PI * i as f64 / opts.points as f64).sin();
          (
            w / 2. + r * a.cos(),
            h / 2. + r * a.sin() + opts.offset * ci as f64,
          )
        })
        .collect::<Vec<(f64, f64)>>();
      rng.shuffle(&mut pts);
      ...
      if ci == colors.len() - 1 {
        l = l.add(signature(1.5, (w - 100., h - 30.), color));
      }
      l
    })
    .collect()
