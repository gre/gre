...
fn art(opts: &Opts) -> Vec<Group> {
  ...
  inks.truncate(rng.gen_range(2, inks.len()));
  ...
  let f1 = rng.gen_range(6.0, 16.0);
  let f2 = rng.gen_range(6.0, 16.0);
  let f3 = rng.gen_range(6.0, 16.0) * rng.gen_range(0.0, 1.0);

  let amp2 = rng.gen_range(0.0, 1.0) * rng.gen_range(0.0, 1.0);

  let linearf = rng.gen_range(0.5, 0.7);
  let noisefactor = rng.gen_range(0.0, 0.5);
  let noisebalance = rng.gen_range(0.3, 0.6);
  ...
  let coloring = |route: &Vec<(f64, f64)>| {
    ...
    let d = route.len() as f64 * width;
    x /= d;
    y /= d;
    let n = 0.5 + 0.5 * perlin.get([2.0 * x, 2.0 * y, 0.7 * opts.seed]);
    let clr = (clrl * n).max(0.0).min(clrl - 1.0) as usize;
    clr
  };
  */
  let mmod = 2 + (rng.gen_range(0., 10.) * rng.gen_range(0., 1.)) as usize;
  let cmod = mmod + rng.gen_range(0, 3) - 1;
