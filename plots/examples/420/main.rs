...
fn art(opts: &Opts) -> Vec<Group> {
  ...
  colors
    ...
    .map(|(_ci, color)| {
      ...
      let mut curve1: Vec<(f64, f64)> = (0..samples)
        .map(|i| {
          let x = pad
            + (i as f64) * (width - 2.0 * pad) / ((samples - 1) as f64)
            + rng.gen_range(-xd, xd) / 2.0;
      ...
      let mut curve2: Vec<(f64, f64)> = (0..samples)
        .map(|i| {
          let x = pad
            + (i as f64) * (width - 2.0 * pad) / ((samples - 1) as f64)
            + rng.gen_range(-xd, xd) / 2.0;
      ...
      loop {
        ...
        let curve: Vec<(f64, f64)> = (1..samples)
          .map(|i| {
            let a = curve1[i];
            let b = curve2[i];
