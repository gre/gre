  ...
  colors
    ...
    .map(|(i, color)| {
      ...
      let mut passage_mm = |p: (f64, f64)| {
        let i =
          (p.1 as usize).max(0).min(199) * 280 + (p.0 as usize).max(0).min(279);
      ...
      let field = |(x, y): (f64, f64), l: f64| {
        4.0 * perlin.get([3.0 * x, 3.0 * y, 1.0 + opts.seed + i as f64 * 0.01])
          + 1.0 * perlin.get([8.0 * x, 8.0 * y, 2.0 + opts.seed + l * 0.05])
      };

      let initial_positions = (0..lines)
        .map(|l| {
          let p = (l as f64 + i as f64 * 0.5) / (lines as f64);
          let amp = radius * (p);
          let a = spins * p * 2. * PI;
          (
            boundaries.0 + (boundaries.2 - boundaries.0) * 0.5 + amp * a.cos(),
            boundaries.1 + (boundaries.3 - boundaries.1) * 0.5 + amp * a.sin(),
          )
        })
        .collect();
      ...
      let mut last_angles: Vec<f64> = (0..lines)
        .map(|l| {
          (spins * ((l as f64 + i as f64 * 0.5) / (lines as f64)) * 2. * PI
            + PI)
            % (2. * PI)
        })
        .collect();
      ...
      let mut build_route = |p: (f64, f64), l, route_i| {
        ...
        let next =
          (p.0 + precision * angle.cos(), p.1 + precision * angle.sin());
        let passage = passage_mm(next);
        let ends = euclidian_dist(next, center) > radius
          || passage > 5
          || i > length
      ...
      let mut routes =
        build_routes_with_collision_par(initial_positions, &mut build_route);
      ...
      for c in 0..1000 {
        ...
        croute.push((center.0 + amp * a.cos(), center.1 + amp * a.sin()));
      }
