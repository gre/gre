...
fn art(opts: Opts) -> Vec<Group> {
  ...
  for (i, color) in colors.iter().enumerate() {
    ...
    let parametric = |p: f64| {
      ...
      let noise_amp = 0.003
        * perlin.get([
          4.8 * p.0 + perlin.get([7.8 * p.0, 4.2 * p.1, 40. + opts.seed]),
          4.8 * p.1
            + 0.8
              * perlin.get([
                4.5 * p.0,
                6.8 * p.1
                  + perlin.get([20.5 * p.0, 20.8 * p.1, 200. + opts.seed]),
                20. + opts.seed,
              ]),
    ...
    l = l.add(signature(0.8, (178.0, 200.0), color));
    layers.push(l);
  }
  layers
}
...
struct Opts {
  ...
  offsety: f64,
  #[clap(short, long, default_value = "130.")]
  size: f64,
  #[clap(short, long, default_value = "0.3")]
  size_diff: f64,
  #[clap(short, long, default_value = "-0.001")]
  offsetx_diff: f64,
  #[clap(short, long, default_value = "0.0001")]
  offsety_diff: f64,
