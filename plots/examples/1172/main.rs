...
fn art(opts: &Opts) -> Vec<Group> {
  ...
  let x = rng.gen_range(0.4, 0.6) * width;
  let y = rng.gen_range(0.4, 0.6) * height;
  let r = rng.gen_range(0.1, 0.2) * width;

  let sun = vec![
    (1, spiral_optimized(x, y, r, 0.4, 0.05)),
    (1, circle_route((x, y), r, (20. + 2. * r) as usize)),
  ];
