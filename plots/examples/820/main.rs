...
fn art(opts: &Opts) -> Vec<Group> {
  ...
  let mut incr = rng.gen_range(0.8, 1.4);
  let incr_mul =
    1.0 + rng.gen_range(-0.01, 0.04) * rng.gen_range(-1.0f64, 1.0).max(0.0);
  let mut p = rng.gen_range(30.0, 50.0);
  let min = rng.gen_range(-100f64, 30.0).max(0.1);
  let mut i = 0;
  let border_count = rng.gen_range(0, 30);
  loop {
    let x2 = width - p;
    let y2 = height - p;
    ...
    p += (incr - rng.gen_range(0.0, 1.0) * rng.gen_range(0.0, 1.0))
      * (if i < border_count { 0.4 } else { 1.0 });
    i += 1;
    if incr > 0.3 {
      incr *= incr_mul;
    }
  ...
  let count = rng.gen_range(4, 14);
  let split = rng.gen_range(0.0, 3.0);
  for _i in 0..count {
    ...
    let slide = rng.gen_range(0.0, 10.0)
