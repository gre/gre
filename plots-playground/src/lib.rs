...
pub fn preserve_ratio_inside((x, y): (f64, f64), (w, h): (f64, f64)) -> (f64, f64) {
    let m = w.min(h);
    let ratio = (w / m, h / m);
    (0.5 + (x - 0.5) * ratio.0, 0.5 + (y - 0.5) * ratio.1)
}

pub fn preserve_ratio_outside((x, y): (f64, f64), (w, h): (f64, f64)) -> (f64, f64) {
    let m = w.max(h);
    let ratio = (w / m, h / m);
    (0.5 + (x - 0.5) * ratio.0, 0.5 + (y - 0.5) * ratio.1)
}
...
pub fn project_in_boundaries(p: (f64, f64), boundaries: (f64, f64, f64, f64)) -> (f64, f64) {
    (
...
pub fn strictly_in_boundaries(p: (f64, f64), boundaries: (f64, f64, f64, f64)) -> bool {
    p.0 > boundaries.0 && p.0 < boundaries.2 && p.1 > boundaries.1 && p.1 < boundaries.3
}
...
pub fn samples_polygon(
    ...
    for x in 0..sz {
        for y in 0..sz {
            ...
            let p: Point<f64> =
                o + point!(x: x as f64 * bounds.width(), y: y as f64 * bounds.height())
                    / (sz as f64);
            if poly.contains(&p) {
...
pub fn tsp(candidates: Vec<(f64, f64)>, duration: Duration) -> Vec<(f64, f64)> {
    let tour = travelling_salesman::simulated_annealing::solve(&candidates, duration);
    return tour.route.iter().map(|&i| candidates[i]).collect();
}

pub fn render_tsp(data: Data, candidates: Vec<(f64, f64)>, duration: Duration) -> Data {
...
pub fn sample_square_voronoi_polys(candidates: Vec<(f64, f64)>, pad: f64) -> Vec<Polygon<f64>> {
...
pub fn group_by_proximity(candidates: Vec<(f64, f64)>, threshold: f64) -> Vec<Vec<(f64, f64)>> {
...
pub fn render_route_curve(data: Data, route: Vec<(f64, f64)>) -> Data {
    ...
    for p in route {
        if first {
            ...
            d = d.quadratic_curve_to((last.0, last.1, (p.0 + last.0) / 2., (p.1 + last.1) / 2.));
        }
        last = p;
    }
    return d;
...
pub fn group_with_kmeans(samples: Vec<(f64, f64)>, n: usize) -> Vec<Vec<(f64, f64)>> {
...
pub fn find_best_collision_1d(from: (f64, f64), others: Vec<(f64, f64)>) -> Option<(f64, f64)> {
...
pub fn rasterize_1d<F: FnMut((f64, f64)) -> f64>(width: u32, height: u32, mut f: F) -> Vec<f64> {
    (0..height)
        .flat_map(|y| {
            (0..width)
...
pub fn features_to_routes(features: Vec<Feature>, precision: f64) -> Vec<Vec<(f64, f64)>> {
