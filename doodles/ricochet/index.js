...
const Main = ({ attributesRef, width, height, random }) => {
  ...
  const ratio = variables.opts.width / variables.opts.height;
  ...
  const renderedSVG = useMemo(
    () =>
      ...
      btoa(
        svg
          .replace(variables.opts.height + "mm", heightPx)
          .replace(variables.opts.width + "mm", widthPx)
      ),
    [svg, widthPx, heightPx, variables.opts]
  );
...
function useVariables({ random }) {
  const vars = useMemo(() => generateVariables(random), []);
  const [max_passage, setMaxPassage] = useState(6);
  useKeypress("v", () => {
    setMaxPassage((m) => {
      const next = m < 3 ? 7 : m - 1;
      console.log("MAX PASSAGE = " + next);
      return next;
    });
  });
  return useMemo(
    () => ({ ...vars, opts: { ...vars.opts, max_passage } }),
    [vars, max_passage]
  );
}
