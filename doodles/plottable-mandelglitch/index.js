...
const Main = ({ width, height, setFeatures, params, hash }) => {
  ...
  const variables = useVariables({ hash, params });
...
function useVariables({ hash, params }) {
  return useMemo(() => generateVariables(hash, params), [hash]);
}
...
const gainShader = `
float gain = smoothstep(0.5, 1.0, abs(cos(${Math.PI}*(abs(p.y-0.5)-0.2*time))));
`;
