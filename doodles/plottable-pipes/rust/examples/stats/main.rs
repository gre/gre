...
fn main() {
  ...
  let mut pipes_counter = HashMap::new();
  ...
  for feature in all {
    ...
    *paper_count += 1;
    ...
    let pipes_count = pipes_counter.entry(feature.pipes.clone()).or_insert(0);
    *pipes_count += 1;
  }
  ...
  println!("Pipes distribution:");
  for (k, v) in pipes_counter.into_iter() {
    println!("{:<30} : {}%", k, (100. * (v as f32)) / (l as f32));
  }
}
