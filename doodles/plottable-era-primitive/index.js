...
const svgSize = [210, 297];
const MAX = 4096;
const ratio = svgSize[0] / svgSize[1];
const svgMMSize = svgSize.map((s) => s + "mm");

let adaptiveSvgWidth = (width) => Math.max(32, Math.ceil(width / 32) * 32);
...
function useVariables({ random }) {
  return useMemo(() => {
    const vars = generateVariables(random, window.fxhash);
    document.body.style.backgroundColor =
      "rgb(" +
      (vars.primary.bg || [1, 1, 1]).map((n) => Math.round(n * 255)).join(",") +
      ")";
...
let pModifierCode = `
p.x += 0.02 *
  smoothstep(0.5, 0.0, p.y) *
  smoothstep(0.46, 0.3, abs(p.x - 0.5)) * (
  cos(40. * p.y + time)
  + sin(10. * p.x + time)
);
`;
...
void main () {
  ...
  main: {
    ...
    void main() {
      ...
      ${pModifierCode};
      float d = abs(p.y-0.5);
      float gain = smoothstep(0.1, 0.0, abs(fract(d - .5 * time) - 0.2));
