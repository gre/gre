...
const svgSize = ["297mm", "210mm"];

const lowResWidth = 600;
const highResWidth = 1200;
const lowResHeight = Math.floor(lowResWidth / ratio);
const highResHeight = Math.floor(highResWidth / ratio);
...
const Main = ({ attributesRef, width, height, random }) => {
  ...
  if (W < 600) {
    widthPx = lowResWidth + "px";
    heightPx = lowResHeight + "px";
  } else {
    widthPx = highResWidth + "px";
    heightPx = highResHeight + "px";
  }
...
const Post = ({
  ...
  return (
    <Node
      ...
      uniforms={{
        t: children,
        paper: <PaperCache width={size.width} seed={paperSeed} grain={256} />,
        time,
        grainAmp: 0.1,
        seed: paperSeed,
...
const shaders = Shaders.create({
  paper: {
    ...
    void main () {
      ...
      pR(p, 2.);
      float t = fbm(seed + p * grain + fbm(-4.0 * p * grain));
  ...
  main: {
    ...
    varying vec2 uv;
    uniform float grainAmp, time, seed;
    ...
    void main() {
      ...
      vec2 q = p.xy - .5;
      float d = length(q);
      float angle = fbm(2. * p + 10. * vec2(cos(0.01 * time), sin(0.01 * time)) + seed);
      p += 0.01 * smoothstep(4.0, 10.0, time) * (0.6 + 0.4 * cos(0.2 * time)) * (1.0 - 0.5 * d) * vec2(cos(7. * angle), sin(7. * angle));
