...
function useVariables({ random }) {
  return useMemo(() => generateVariables(random, fxhash), []);
}
...
function useTime(ready, modulo) {
  ...
  useEffect(() => {
    ...
    function loop(t) {
      ...
      const i = Math.floor(time * modulo) % modulo;
      setI(i);
    }
    h = requestAnimationFrame(loop);
...
const Post = ({
  ...
  const i = useTime(ready, frames);
  const crop = useMemo(() => {
    const x = (i % grid[0]) / grid[0];
  ...
  const background = primary.bg || [1, 1, 1];
