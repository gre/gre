...
const Main = ({ attributesRef, width, height, random }) => {
  ...
  const renderedSVG = useMemo(
    () =>
      ...
      btoa(svg.replace("210mm", heightPx).replace("210mm", widthPx)),
...
const Paper = ({ seed, grain, time }) => (
  <Node
    shader={shaders.paper}
    uniforms={{ seed, grain, time, resolution: Uniform.Resolution }}
  />
);
...
const shaders = Shaders.create({
  paper: {
    ...
    void main () {
      ...
      float print = smoothstep(0.85, 0.86, fbm(100.0 * cos(vec2(time)) - seed + 9. * p + 0.8 * fbm(12. * p - 99. * sin(vec2(time)))));
      gl_FragColor = vec4(vec3(t, t, print), 1.0);
    }`,
  },
  main: {
    frag: GLSL`
    ...
    void main() {
      ...
      vec2 q = p.xy - .5;
      float d = length(q);
      float gain = 0.1 * smoothstep(0.2, 0.0, abs(fract(d - 0.5 * time) - 0.4));
