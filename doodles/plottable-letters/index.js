...
import generateVariables, { bakeImage } from "./variables";
...
const fontLoad = new FontFace("font", "url(font.ttf)").load();
const promiseOfLoad = Promise.all([init(decode(wasm)), fontLoad]).then(() => {
  wasmLoaded = true;
});
...
const Main = ({ attributesRef, width, height, random }) => {
  ...
  const svg = useMemo(() => {
    if (!loaded) return "";
    let prev = Date.now();
    const image = bakeImage(variables.bakeImageOpts);
    const result = render({ ...variables.opts, image });
    console.log(
      "svg calc time = " +
        (Date.now() - prev) +
        "ms – " +
